...
type
  ...
  TFastMM_MemoryManagerOptimizationStrategy = (mmosOptimizeForSpeed,
    mmosBalanced, mmosOptimizeForLowMemoryUsage);
...
{$ENDIF}
  ...
  TFastMM_WalkAllocatedBlocksBlockType = (btLargeBlock, btMediumBlockSpan,
    btMediumBlock, btSmallBlockSpan, btSmallBlock);
  ...
  TFastMM_WalkBlocksCallback = procedure(const ABlockInfo
    : TFastMM_WalkAllocatedBlocks_BlockInfo);
  ...
  TFastMM_MinimumAddressAlignment = (maa8Bytes, maa16Bytes, maa32Bytes,
    maa64Bytes);
  ...
  TFastMM_GetStackTrace = procedure(APReturnAddresses: PNativeUInt;
  ...
  TFastMM_ConvertStackTraceToText = function(APReturnAddresses: PNativeUInt;
    AMaxDepth: Cardinal; APBuffer, APBufferEnd: PWideChar): PWideChar;

  { The interface for the legacy (version 4) stack trace conversion routine in the FastMM_FullDebugMode library. }
  TFastMM_LegacyConvertStackTraceToText = function(APReturnAddresses
    : PNativeUInt; AMaxDepth: Cardinal; APBuffer: PAnsiChar): PAnsiChar;
...
function FastMM_DebugReallocMem(APointer: Pointer; ANewSize: NativeInt)
  : Pointer;
...
function FastMM_RegisterExpectedMemoryLeak(ALeakedPointer: Pointer)
  : Boolean; overload;
function FastMM_RegisterExpectedMemoryLeak(ALeakedObjectClass: TClass;
  ACount: Integer = 1): Boolean; overload;
function FastMM_RegisterExpectedMemoryLeak(ALeakedBlockSize: NativeInt;
  ACount: Integer = 1): Boolean; overload;
{ Removes expected memory leaks.  Returns True on success. }
function FastMM_UnregisterExpectedMemoryLeak(ALeakedPointer: Pointer)
  : Boolean; overload;
function FastMM_UnregisterExpectedMemoryLeak(ALeakedObjectClass: TClass;
  ACount: Integer = 1): Boolean; overload;
function FastMM_UnregisterExpectedMemoryLeak(ALeakedBlockSize: NativeInt;
  ACount: Integer = 1): Boolean; overload;
...
function FastMM_WalkBlocks(ACallBack: TFastMM_WalkBlocksCallback;
  AWalkBlockTypes: TFastMM_WalkBlocksBlockTypes = [];
...
function FastMM_DetectStringData(APMemoryBlock: Pointer;
  AAvailableSpaceInBlock: NativeInt): TFastMM_StringDataType;
...
function FastMM_ScanDebugBlocksForCorruption(ALockTimeoutMilliseconds
  : Cardinal = 50): Boolean;
...
function FastMM_GetHeapStatus(ALockTimeoutMilliseconds: Cardinal = 50)
  : THeapStatus;
...
function FastMM_GetUsageSummary(ALockTimeoutMilliseconds: Cardinal = 50)
  : TFastMM_UsageSummary;
...
{ Writes a log file containing a summary of the memory manager state and a list of allocated blocks grouped by class.
  ...
  success. }
function FastMM_LogStateToFile(const AFilename: string;
  const AAdditionalDetails: string = '';
...
procedure FastMM_SetOptimizationStrategy(AStrategy
  : TFastMM_MemoryManagerOptimizationStrategy);
function FastMM_GetCurrentOptimizationStrategy
  : TFastMM_MemoryManagerOptimizationStrategy;
...
procedure FastMM_EnterMinimumAddressAlignment(AMinimumAddressAlignment
  : TFastMM_MinimumAddressAlignment);
procedure FastMM_ExitMinimumAddressAlignment(AMinimumAddressAlignment
  : TFastMM_MinimumAddressAlignment);
{ Returns the current minimum address alignment in effect. }
function FastMM_GetCurrentMinimumAddressAlignment
  : TFastMM_MinimumAddressAlignment;
...
procedure FastMM_NoOpGetStackTrace(APReturnAddresses: PNativeUInt;
  AMaxDepth, ASkipFrames: Cardinal);
function FastMM_NoOpConvertStackTraceToText(APReturnAddresses: PNativeUInt;
  AMaxDepth: Cardinal; APBufferPosition, APBufferEnd: PWideChar): PWideChar;
...
var
  ...
  FastMM_DebugSupportLibraryName
    : PWideChar = CFastMM_DefaultDebugSupportLibraryName;
  ...
  FastMM_OutputDebugStringEvents: TFastMM_MemoryManagerEventTypeSet =
    [mmetDebugBlockDoubleFree, mmetDebugBlockReallocOfFreedBlock,
    mmetDebugBlockHeaderCorruption, mmetDebugBlockFooterCorruption,
  ...
  FastMM_LogToFileEvents: TFastMM_MemoryManagerEventTypeSet =
    [mmetDebugBlockDoubleFree, mmetDebugBlockReallocOfFreedBlock,
    mmetDebugBlockHeaderCorruption, mmetDebugBlockFooterCorruption,
    mmetDebugBlockModifiedAfterFree, mmetVirtualMethodCallOnFreedObject,
    mmetAnotherThirdPartyMemoryManagerAlreadyInstalled,
  ...
  FastMM_LogFileEntryHeader
    : PWideChar =
    '--------------------------------{1} {2}--------------------------------'#13#10;
  { Memory manager installation errors }
  FastMM_CannotInstallAfterDefaultMemoryManagerHasBeenUsedMessage
    : PWideChar = 'FastMM cannot be installed, because the ' +
    'default memory manager has already been used to allocate memory.';
  FastMM_CannotSwitchToSharedMemoryManagerWithLivePointersMessage
    : PWideChar = 'Cannot switch to the shared memory ' +
    'manager, because the local memory manager instance has already been used to allocate memory.';
  FastMM_AnotherMemoryManagerAlreadyInstalledMessage
    : PWideChar = 'FastMM cannot be installed, because another third ' +
    'party memory manager has already been installed.';
  FastMM_CannotSwitchMemoryManagerMessageBoxCaption
    : PWideChar = 'Cannot Switch Memory Managers';

  { Memory leak messages. }
  FastMM_MemoryLeakDetailMessage_NormalBlock
    : PWideChar = 'A memory block has been leaked. The size is: {3}'#13#10#13#10
    ...
    + '{12}'#13#10'{13}'#13#10;
  FastMM_MemoryLeakDetailMessage_DebugBlock
    : PWideChar = 'A memory block has been leaked. The size is: {3}'#13#10#13#10
    ...
    'Current memory dump of {10} bytes starting at pointer address {11}:'#13#10
    + '{12}'#13#10'{13}'#13#10;
  FastMM_MemoryLeakSummaryMessage_LeakDetailNotLogged
    : PWideChar = 'This application has leaked memory. ' +
  ...
  FastMM_DebugBlockDoubleFree
    : PWideChar =
    'An attempt was made to free a block that has already been freed.'#13#10#13#10
    ...
    + '{7}'#13#10#13#10 + 'The allocation number is: {9}'#13#10;
  FastMM_DebugBlockReallocOfFreedBlock
    : PWideChar =
    'An attempt was made to resize a block that has already been freed.'#13#10#13#10
    + 'The block size is {3}.'#13#10#13#10 +
    'The block was allocated by thread 0x{4}, and the stack trace (return addresses) at the time was:'
    + '{6}'#13#10#13#10'This block was freed by thread 0x{5}, and the stack trace (return addresses) at the time was:'
    + '{7}'#13#10#13#10 + 'The allocation number is: {9}'#13#10;
  ...
  FastMM_BlockModifiedAfterFreeMessage
    : PWideChar = 'A memory block was modified after it was freed.'#13#10#13#10
    + 'The block size is {3}.'#13#10#13#10 +
    'Modifications were detected at offsets (with lengths in brackets): {15}.'#13#10#13#10
    + 'The block was allocated by thread 0x{4}, and the stack trace (return addresses) at the time was:'
    + '{6}'#13#10#13#10'This block was freed by thread 0x{5}, and the stack trace (return addresses) at the time was:'
    + '{7}'#13#10#13#10 + 'The allocation number is: {9}'#13#10#13#10 +
    'Current memory dump of {10} bytes starting at pointer address {11}:'#13#10
    + '{12}'#13#10'{13}'#13#10;
  FastMM_BlockHeaderCorruptedMessage
    : PWideChar = 'A memory block header has been corrupted.'#13#10#13#10 +
    'Current memory dump of {10} bytes starting at pointer address {11}:'#13#10
    + '{12}'#13#10'{13}'#13#10;
  FastMM_BlockFooterCorruptedMessage_AllocatedBlock
    : PWideChar = 'A memory block footer has been corrupted.'#13#10#13#10 +
    'The block size is {3}.'#13#10#13#10 +
    'The block was allocated by thread 0x{4}, and the stack trace (return addresses) at the time was:'
    + '{6}'#13#10#13#10 + 'The allocation number is: {9}'#13#10#13#10 +
    'Current memory dump of {10} bytes starting at pointer address {11}:'#13#10
    + '{12}'#13#10'{13}'#13#10;
  FastMM_BlockFooterCorruptedMessage_FreedBlock
    : PWideChar = 'A memory block footer has been corrupted.'#13#10#13#10 +
    'The block size is {3}.'#13#10#13#10 +
    'The block was allocated by thread 0x{4}, and the stack trace (return addresses) at the time was:'
    + '{6}'#13#10#13#10'This block was freed by thread 0x{5}, and the stack trace (return addresses) at the time was:'
    ...
    'Current memory dump of {10} bytes starting at pointer address {11}:'#13#10
    + '{12}'#13#10'{13}'#13#10;
  FastMM_MemoryCorruptionMessageBoxCaption
    : PWideChar = 'Memory Corruption Detected';

  { Virtual method call on a freed object. }
  FastMM_VirtualMethodCallOnFreedObjectMessage
    : PWideChar = 'A virtual method was called on a freed object.'#13#10#13#10 +
    'Freed object class: {8}'#13#10#13#10 + 'Virtual method: {17}'#13#10#13#10 +
    'The block size is {3}.'#13#10#13#10 +
    'The block was allocated by thread 0x{4}, and the stack trace (return addresses) at the time was:'
    + '{6}'#13#10#13#10'This block was freed by thread 0x{5}, and the stack trace (return addresses) at the time was:'
    + '{7}'#13#10#13#10'The stack trace for the virtual call that lead to this error is:'
    + '{24}'#13#10#13#10 + 'The allocation number is: {9}'#13#10#13#10 +
    'Current memory dump of {10} bytes starting at pointer address {11}:'#13#10
    + '{12}'#13#10'{13}'#13#10;
  FastMM_VirtualMethodCallOnFreedObjectMessageBoxCaption
    : PWideChar = 'Virtual Method Call On Freed Object';

  { Memory state logging messages }
  FastMM_LogStateToFileTemplate: PWideChar = 'FastMM State Capture:'#13#10 +
    '---------------------'#13#10 + '{18}K Allocated'#13#10 +
    '{19}K Overhead'#13#10 + '{20}% Efficiency'#13#10#13#10 +
    'Usage Detail:'#13#10;
  FastMM_LogStateToFileTemplate_UsageDetail
    : PWideChar = '{21} bytes: {8} x {22} ({23} bytes avg.)'#13#10;
...
{$ELSE}
procedure DebugLibrary_GetRawStackTrace(APReturnAddresses: PNativeUInt;
  AMaxDepth, ASkipFrames: Cardinal);
  external CFastMM_DefaultDebugSupportLibraryName name 'GetRawStackTrace';
procedure DebugLibrary_GetFrameBasedStackTrace(APReturnAddresses: PNativeUInt;
  AMaxDepth, ASkipFrames: Cardinal);
  external CFastMM_DefaultDebugSupportLibraryName name
  'GetFrameBasedStackTrace';
function DebugLibrary_LogStackTrace_Legacy(APReturnAddresses: PNativeUInt;
  AMaxDepth: Cardinal; APBuffer: PAnsiChar): PAnsiChar;
  external CFastMM_DefaultDebugSupportLibraryName name 'LogStackTrace';
{$ENDIF}
...
{$ENDIF}
  ...
  // Must be a multiple of 64 for the 64-byte alignment option to work
  ...
  CMediumBlockMiddleBinsStart = CMinimumMediumBlockSize + CInitialBinSpacing *
    CInitialBinCount;
  CMiddleBinCount = 384;
  CMiddleBinSpacingBits = 9;
  CMiddleBinSpacing = 1 shl CMiddleBinSpacingBits; // = 512

  CMediumBlockFinalBinsStart = CMediumBlockMiddleBinsStart + CMiddleBinSpacing *
    CMiddleBinCount;
  ...
  CMaximumMediumBlockSize = CMediumBlockFinalBinsStart + (CFinalBinCount - 1) *
    CFinalBinSpacing;

  { -----Large block constants----- }
  CLargeBlockGranularity = 64 * 1024;
  // Address space obtained from VirtualAlloc is always aligned to a 64K boundary
...
type
  ...
  TFastMM_MaximumLengthStackTrace = array
    [0 .. CFastMM_StackTrace_MaximumEntryCount - 1] of NativeUInt;
  ...
  TIntegerWithABACounter = record
    case Integer of
      0:
        (IntegerAndABACounter: Int64);
      1:
        (IntegerValue, ABACounter: Integer);
  end;
...
{$ENDIF}
  ...
  TSmallBlockArena = array [0 .. CSmallBlockTypeCount - 1]
    of TSmallBlockManager;
  PSmallBlockArena = ^TSmallBlockArena;

  TSmallBlockArenas = array [0 .. CFastMM_SmallBlockArenaCount - 1]
    of TSmallBlockArena;
...
{$POINTERMATH Off}
  ...
  TMediumBlockSpanHeader = packed record
    ...
    PreviousMediumBlockSpanHeader: PMediumBlockSpanHeader;
...
{$POINTERMATH Off}
  ...
  TMediumBlockManager = record
    ...
    FirstMediumBlockSpanHeader: PMediumBlockSpanHeader;
    // Do not change position
    ...
    MediumBlockBinBitmaps: array [0 .. CMediumBlockBinGroupCount - 1]
      of Cardinal;
  ...
  TMediumBlockArenas = array [0 .. CFastMM_MediumBlockArenaCount - 1]
    of TMediumBlockManager;
...
{$ENDIF}
  ...
  TLargeBlockArenas = array [0 .. CFastMM_LargeBlockArenaCount - 1]
    of TLargeBlockManager;
  ...
  TExpectedMemoryLeaks = record
    ...
    ExpectedLeaks: array [0 .. (CExpectedMemoryLeaksListSize - 64)
  ...
  TMemoryLeakSummaryEntry = record
    ...
    // 0 = unknown, 1 = AnsiString, 2 = UnicodeString, other values = class pointer
  ...
  public
    ...
    procedure VirtualMethod4; virtual;
    procedure VirtualMethod5; virtual;
    procedure VirtualMethod6; virtual;
    procedure VirtualMethod7; virtual;
    procedure VirtualMethod8; virtual;
    procedure VirtualMethod9; virtual;
    procedure VirtualMethod10; virtual;
    procedure VirtualMethod11; virtual;
    procedure VirtualMethod12; virtual;
    procedure VirtualMethod13; virtual;
    procedure VirtualMethod14; virtual;
    procedure VirtualMethod15; virtual;
    procedure VirtualMethod16; virtual;
    procedure VirtualMethod17; virtual;
    procedure VirtualMethod18; virtual;
    procedure VirtualMethod19; virtual;
    procedure VirtualMethod20; virtual;
    procedure VirtualMethod21; virtual;
    procedure VirtualMethod22; virtual;
    procedure VirtualMethod23; virtual;
    procedure VirtualMethod24; virtual;
    procedure VirtualMethod25; virtual;
    procedure VirtualMethod26; virtual;
    procedure VirtualMethod27; virtual;
    procedure VirtualMethod28; virtual;
    procedure VirtualMethod29; virtual;
    procedure VirtualMethod30; virtual;
    procedure VirtualMethod31; virtual;
    procedure VirtualMethod32; virtual;
    procedure VirtualMethod33; virtual;
    procedure VirtualMethod34; virtual;
    procedure VirtualMethod35; virtual;
    procedure VirtualMethod36; virtual;
    procedure VirtualMethod37; virtual;
    procedure VirtualMethod38; virtual;
    procedure VirtualMethod39; virtual;
    procedure VirtualMethod40; virtual;
    procedure VirtualMethod41; virtual;
    procedure VirtualMethod42; virtual;
    procedure VirtualMethod43; virtual;
    procedure VirtualMethod44; virtual;
    procedure VirtualMethod45; virtual;
    procedure VirtualMethod46; virtual;
    procedure VirtualMethod47; virtual;
    procedure VirtualMethod48; virtual;
    procedure VirtualMethod49; virtual;
    procedure VirtualMethod50; virtual;
    procedure VirtualMethod51; virtual;
    procedure VirtualMethod52; virtual;
    procedure VirtualMethod53; virtual;
    procedure VirtualMethod54; virtual;
    procedure VirtualMethod55; virtual;
    procedure VirtualMethod56; virtual;
    procedure VirtualMethod57; virtual;
    procedure VirtualMethod58; virtual;
    procedure VirtualMethod59; virtual;
    procedure VirtualMethod60; virtual;
    procedure VirtualMethod61; virtual;
    procedure VirtualMethod62; virtual;
    procedure VirtualMethod63; virtual;
    procedure VirtualMethod64; virtual;
    procedure VirtualMethod65; virtual;
    procedure VirtualMethod66; virtual;
    procedure VirtualMethod67; virtual;
    procedure VirtualMethod68; virtual;
    procedure VirtualMethod69; virtual;
    procedure VirtualMethod70; virtual;
    procedure VirtualMethod71; virtual;
    procedure VirtualMethod72; virtual;
    procedure VirtualMethod73; virtual;
    procedure VirtualMethod74; virtual;
  end;
...
{$ENDIF}
    ...
    { 128 byte jumps }
    1472, 1600, 1728, 1856, 1984, 2112, 2240, 2368, 2496, CMaximumSmallBlockSize
    // = 2624
    );

var
  { Lookup table for converting a block size to a small block type index from 0..CSmallBlockTypeCount - 1 }
  SmallBlockTypeLookup
    : array [0 .. CMaximumSmallBlockSize div CSmallBlockGranularity -
    1] of Byte;
...
function TFastMM_DebugBlockHeader.DebugFooter_AllocationStackTracePtr
  : PNativeUInt;
begin
  { The allocation stack trace follows immediately after the footer checksum. }
  Result := @PByte(@Self)
    [UserSize + (CDebugBlockHeaderSize + CDebugBlockFooterCheckSumSize)];
end;
...
procedure MoveMultipleOf16_x86_SSE2(const ASource; var ADest;
  ACount: NativeInt);
asm
  add eax, ecx
  add edx, ecx
...
procedure MoveMultipleOf32_x86_SSE2(const ASource; var ADest;
  ACount: NativeInt);
asm
  add eax, ecx
  add edx, ecx
...
procedure MoveMultipleOf64_Small_x86_SSE2(const ASource; var ADest;
  ACount: NativeInt);
asm
  add eax, ecx
  add edx, ecx
...
function OS_FreeVirtualMemory(APointer: Pointer; ABlockSize: NativeInt)
  : Integer;
begin
...
function OS_AllocateVirtualMemory(ABlockSize: NativeInt;
...
begin
  ...
  begin
    Result := Winapi.Windows.VirtualAlloc(nil, NativeUInt(ABlockSize),
  ...
  begin
    ...
    Result := Winapi.Windows.VirtualAlloc(nil, NativeUInt(ABlockSize),
      LAllocationFlags, PAGE_READWRITE);
  ...
  begin
    ...
    if (MemoryUsageLimit > 0) and (MemoryUsageCurrent > MemoryUsageLimit) then
    begin
...
function OS_AllocateVirtualMemoryAtAddress(APAddress: Pointer;
  ABlockSize: NativeInt; AReserveOnlyNoReadWriteAccess: Boolean): Boolean;
begin
  if AReserveOnlyNoReadWriteAccess then
  begin
    Result := Winapi.Windows.VirtualAlloc(APAddress, NativeUInt(ABlockSize),
      MEM_RESERVE, PAGE_NOACCESS) <> nil;
  end
  else
  begin
    Result := (Winapi.Windows.VirtualAlloc(APAddress, NativeUInt(ABlockSize),
      MEM_RESERVE, PAGE_READWRITE) <> nil) and
      (Winapi.Windows.VirtualAlloc(APAddress, NativeUInt(ABlockSize),
      MEM_COMMIT, PAGE_READWRITE) <> nil);
  end;
...
procedure OS_GetVirtualMemoryRegionInfo(APRegionStart: Pointer;
  var AMemoryRegionInfo: TMemoryRegionInfo);
var
  LMemInfo: TMemoryBasicInformation;
begin
  if Winapi.Windows.VirtualQuery(APRegionStart, LMemInfo, SizeOf(LMemInfo)) > 0
  then
  begin
    ...
    if (LMemInfo.State = MEM_COMMIT) and (LMemInfo.Protect and PAGE_GUARD = 0)
    then
    begin
      if (LMemInfo.Protect and (PAGE_READONLY or PAGE_READWRITE or
        PAGE_EXECUTE or PAGE_EXECUTE_READ or PAGE_EXECUTE_READWRITE or
        PAGE_EXECUTE_WRITECOPY) <> 0) then
        Include(AMemoryRegionInfo.AccessRights, marRead);
      if (LMemInfo.Protect and (PAGE_READWRITE or PAGE_EXECUTE_READWRITE or
        PAGE_EXECUTE_WRITECOPY) <> 0) then
        Include(AMemoryRegionInfo.AccessRights, marWrite);
      if (LMemInfo.Protect and (PAGE_EXECUTE or PAGE_EXECUTE_READ or
        PAGE_EXECUTE_READWRITE or PAGE_EXECUTE_WRITECOPY) <> 0) then
...
procedure OS_GetCurrentDateTime(var AYear, AMonth, ADay, AHour, AMinute,
  ASecond, AMilliseconds: Word);
var
...
function OS_GetApplicationFilename(APFilenameBuffer, APBufferEnd: PWideChar;
  AReturnLibraryFilename: Boolean): PWideChar;
var
...
begin
  ...
  LNumChars := Winapi.Windows.GetModuleFileNameW(LModuleHandle, Result,
    Cardinal(CharCount(APBufferEnd, APFilenameBuffer)));
  Inc(Result, LNumChars);
end;

function OS_GetEnvironmentVariableValue(APEnvironmentVariableName,
  APValueBuffer, APBufferEnd: PWideChar): PWideChar;
var
...
begin
  ...
  LBufferSize := (NativeUInt(APBufferEnd) - NativeUInt(Result))
    div SizeOf(WideChar);
  LNumChars := Winapi.Windows.GetEnvironmentVariableW(APEnvironmentVariableName,
    Result, LBufferSize);
...
begin
  ...
  Result := Winapi.Windows.GetFileAttributesW(APFileName) <>
...
function OS_OpenOrCreateFile(APFileName: PWideChar;
  var AFileHandle: THandle): Boolean;
begin
  { Try to open/create the file in read/write mode. }
  AFileHandle := Winapi.Windows.CreateFileW(APFileName, GENERIC_READ or
    GENERIC_WRITE, FILE_SHARE_READ, nil, OPEN_ALWAYS, FILE_ATTRIBUTE_NORMAL, 0);
...
function OS_WriteFile(AFileHandle: THandle; APData: Pointer;
  ADataSizeInBytes: Integer): Boolean;
var
  LBytesWritten: Cardinal;
begin
  Winapi.Windows.WriteFile(AFileHandle, APData^, Cardinal(ADataSizeInBytes),
    LBytesWritten, nil);
...
begin
  Winapi.Windows.MessageBoxW(0, APText, APCaption, MB_OK or MB_ICONERROR or
    MB_TASKMODAL or MB_DEFAULT_DESKTOP_ONLY);
end;
...
begin
  Result := (NativeInt(APFirstFreeChar) - NativeInt(APBufferStart))
    div SizeOf(WideChar);
end;
...
function ConvertUTF16toUTF8(APWideText: PWideChar; ANumWideChars: Integer;
  APOutputBuffer: PByte): PByte;
var
  LPIn, LPEnd: PWord;
  LCode: Cardinal;
begin
      ...
      begin
        ...
        begin
          { It is a surrogate pair (4 byte) encoding:  Surrogate pairs are encoded in four bytes, with the high word
            first }
          LCode := ((LCode and $3FF) shl 10) +
            ((LCode shr 16) and $3FF) + $10000;
...
function OpenOrCreateTextFile(APFileName: PWideChar;
  AAddLineBreakToExistingFile: Boolean; var AFileHandle: THandle): Boolean;
const
...
function AppendTextFile(AFileHandle: THandle; APText: PWideChar;
  AWideCharCount: Integer): Boolean;
var
  LBufferSize: Integer;
...
begin
  ...
  try
    ...
    Result := OS_WriteFile(AFileHandle, LPBufferStart, NativeInt(LPBufferPos) -
      NativeInt(LPBufferStart));
...
var
  ...
  begin
    { Do some basic pointer checks:  Must be pointer aligned and beyond 64K. (The low 64K is never readable, at least
      under Windows.) }
    if (NativeUInt(APAddress) <= 65535) or
      (NativeUInt(APAddress) and (SizeOf(Pointer) - 1) <> 0) then
    begin
      Exit(False);
    end;
    ...
    if (NativeUInt(APAddress) < NativeUInt(LMemoryRegionInfo.RegionStartAddress)
      ) or (NativeUInt(APAddress) -
      NativeUInt(LMemoryRegionInfo.RegionStartAddress) >=
      LMemoryRegionInfo.RegionSize) then
    begin
      OS_GetVirtualMemoryRegionInfo(APAddress, LMemoryRegionInfo);
    end;

    { The address must be readable. }
    Result := (not LMemoryRegionInfo.RegionIsFree) and
      (marRead in LMemoryRegionInfo.AccessRights);
  end;
...
{ Returns True if AClassPointer points to a class VMT }
  ...
  begin
    ...
    if (ADepth < 1000) and (NativeUInt(AClassPointer) > 65535) and
      IsValidVMTAddress(Pointer(PByte(AClassPointer) + vmtSelfPtr)) and
    ...
    begin
      ...
      Result := IsValidVMTAddress(LParentClassSelfPointer) and
...
begin
  ...
  if (Result = TFastMM_FreedObject) or
    (not InternalIsValidClass(Pointer(Result), 0)) then
    Result := nil;
end;
...
begin
  ...
  if (LStringLength <= 0) or
    (LStringLength >= (AAvailableSpaceInBlock - SizeOf(StrRec)) div LElementSize)
  then
  begin
    Exit(sdtNotAString);
  end;
...
{ Attempts to detect the class or string type of the given block.  Possible return values are:
  ...
  > 1 = TClass Pointer }
function DetectBlockContentType(APMemoryBlock: Pointer;
  AAvailableSpaceInBlock: NativeInt): NativeUInt;
var
  LLeakedClass: TClass;
...
function AppendTextToBuffer(APSource, APTarget, APTargetBufferEnd: PWideChar)
  : PWideChar; overload;
var
  LChar: WideChar;
begin
...
function NativeUIntToHexadecimalBuffer(AValue: NativeUInt;
  APTarget, APTargetBufferEnd: PWideChar): PWideChar;
var
...
function NativeUIntToTextBuffer(AValue: NativeUInt;
  APTarget, APTargetBufferEnd: PWideChar): PWideChar;
var
...
function NativeIntToTextBuffer(AValue: NativeInt;
  APTarget, APTargetBufferEnd: PWideChar): PWideChar;
const
...
begin
  ...
  Result := NativeUIntToTextBuffer(NativeUInt(Abs(AValue)), Result,
    APTargetBufferEnd);
end;

function BlockContentTypeToTextBuffer(ABlockContentType: NativeUInt;
  APTarget, APTargetBufferEnd: PWideChar): PWideChar;
type
...
begin
  ...
  case ABlockContentType of
    0:
      Result := AppendTextToBuffer(CUnknown, length(CUnknown), Result,
        APTargetBufferEnd);
    1:
      Result := AppendTextToBuffer(CAnsiString, length(CAnsiString), Result,
        APTargetBufferEnd);
    2:
      Result := AppendTextToBuffer(CUnicodeString, length(CUnicodeString),
        Result, APTargetBufferEnd);
    ...
    begin
      ...
      begin
        LPShortString := @PClassData(PByte(LClassInfo) + 2 +
          PByte(PByte(LClassInfo) + 1)^).UnitName;
      ...
      Result := AppendTextToBuffer(@LBuffer, CharCount(LPTarget, @LBuffer),
        Result, APTargetBufferEnd);
    end;
...
{ Copies a token value to the buffer and sets the pointer to the token in the values array.  Copies up to the size of
  the target buffer. }
function AddTokenValue(var ATokenValues: TEventLogTokenValues;
  ATokenID: Integer; APTokenValue: PWideChar; ACharCount: Integer;
...
begin
  ...
  if (ACharCount <= 0) or (@Result[ACharCount] >= APBufferEnd) then
  begin
...
function AddTokenValue_NativeInt(var ATokenValues: TEventLogTokenValues;
  ATokenID: Integer; ATokenValue: NativeInt;
...
begin
  ...
  LPPos := NativeIntToTextBuffer(ATokenValue, @LTempBuffer,
    @LTempBuffer[High(LTempBuffer)]);

  Result := AddTokenValue(ATokenValues, ATokenID, @LTempBuffer,
    CharCount(LPPos, @LTempBuffer), Result, APBufferEnd);
end;

function AddTokenValue_NativeUInt(var ATokenValues: TEventLogTokenValues;
  ATokenID: Integer; ATokenValue: NativeUInt;
  APTokenValueBufferPos, APBufferEnd: PWideChar): PWideChar;
var
  LTempBuffer: array [0 .. 20] of WideChar;
  LPPos: PWideChar;
begin
  ...
  LPPos := NativeUIntToTextBuffer(ATokenValue, @LTempBuffer,
    @LTempBuffer[High(LTempBuffer)]);
...
function AddTokenValue_Hexadecimal(var ATokenValues: TEventLogTokenValues;
  ATokenID: Integer; ATokenValue: NativeUInt;
  APTokenValueBufferPos, APBufferEnd: PWideChar): PWideChar;
var
  LTempBuffer: array [0 .. 15] of WideChar;
...
begin
  ...
  LPPos := NativeUIntToHexadecimalBuffer(ATokenValue, @LTempBuffer,
    @LTempBuffer[High(LTempBuffer)]);
...
function AddTokenValue_HexDump(var ATokenValues: TEventLogTokenValues;
  ATokenID: Integer; APBlock: PByte; ANumBytes: Integer;
  APTokenValueBufferPos, APBufferEnd: PWideChar): PWideChar;
var
  LTempBuffer: array [0 .. CMemoryDumpMaxBytes * 5] of WideChar;
  // Worst case scenario:  Allow for CRLF after every byte
  LPTarget: PWideChar;
...
begin
  ...
  Result := AddTokenValue(ATokenValues, ATokenID, @LTempBuffer,
    CharCount(LPTarget, @LTempBuffer), Result, APBufferEnd);
end;

function AddTokenValue_ASCIIDump(var ATokenValues: TEventLogTokenValues;
  ATokenID: Integer; APBlock: PByte; ANumBytes: Integer;
  APTokenValueBufferPos, APBufferEnd: PWideChar): PWideChar;
var
  LTempBuffer: array [0 .. CMemoryDumpMaxBytes * 5] of WideChar;
...
function AddTokenValue_StackTrace(var ATokenValues: TEventLogTokenValues;
  ATokenID: Integer; APStackTrace: PNativeUInt; AStackTraceEntryCount: Cardinal;
  APTokenValueBufferPos, APBufferEnd: PWideChar): PWideChar;
var
  LStackTraceBuffer: array [0 .. CFastMM_StackTrace_MaximumEntryCount * 160]
    of WideChar;
  LPBuffer: PWideChar;
begin
  Result := APTokenValueBufferPos;
  ...
  begin
    LPBuffer := FastMM_ConvertStackTraceToText(APStackTrace,
      AStackTraceEntryCount, LPBuffer,
      @LStackTraceBuffer[High(LStackTraceBuffer)]);
  end;

  Result := AddTokenValue(ATokenValues, ATokenID, LStackTraceBuffer,
    CharCount(LPBuffer, @LStackTraceBuffer), Result, APBufferEnd);
end;

{ Adds a date token in ISO 8601 date format, e.g. 2020-01-01 }
function AddTokenValue_Date(var ATokenValues: TEventLogTokenValues;
  ATokenID: Integer; AYear, AMonth, ADay: Word;
  APTokenValueBufferPos, APBufferEnd: PWideChar): PWideChar;
var
...
begin
  ...
  Result := AddTokenValue(ATokenValues, ATokenID, @LDateBuffer,
    length(LDateBuffer), Result, APBufferEnd);
end;

{ Adds a date token in ISO 8601 date format, e.g. 2020-01-01 }
function AddTokenValue_Time(var ATokenValues: TEventLogTokenValues;
  ATokenID: Integer; AHour, AMinute, ASecond: Word;
  APTokenValueBufferPos, APBufferEnd: PWideChar): PWideChar;
var
...
begin
  ...
  Result := AddTokenValue(ATokenValues, ATokenID, @LTimeBuffer,
    length(LTimeBuffer), Result, APBufferEnd);
end;

{ Adds the tokens for the current date and time. }
function AddTokenValues_CurrentDateAndTime(var ATokenValues
  : TEventLogTokenValues; APTokenValueBufferPos, APBufferEnd: PWideChar)
  : PWideChar;
var
...
begin
  ...
  Result := AddTokenValue_Date(ATokenValues, CEventLogTokenCurrentDate, LYear,
    LMonth, LDay, Result, APBufferEnd);
  Result := AddTokenValue_Time(ATokenValues, CEventLogTokenCurrentTime, LHour,
    LMinute, LSecond, Result, APBufferEnd);
end;

function AddTokenValue_BlockContentType(var ATokenValues: TEventLogTokenValues;
  ATokenID: Integer; ABlockContentType: NativeUInt;
  APTokenValueBufferPos, APBufferEnd: PWideChar): PWideChar;
const
...
begin
  ...
  LPPos := BlockContentTypeToTextBuffer(ABlockContentType, @LBuffer,
    @LBuffer[High(LBuffer)]);

  Result := AddTokenValue(ATokenValues, ATokenID, @LBuffer,
    CharCount(LPPos, @LBuffer), Result, APBufferEnd);
end;

function AddTokenValues_GeneralTokens(var ATokenValues: TEventLogTokenValues;
  APTokenValueBufferPos, APBufferEnd: PWideChar): PWideChar;
begin
  Result := AddTokenValues_CurrentDateAndTime(ATokenValues,
    APTokenValueBufferPos, APBufferEnd);
  Result := AddTokenValue(ATokenValues, CEventLogTokenEventLogFilename,
    @EventLogFilename, GetStringLength(@EventLogFilename), Result, APBufferEnd);
end;

function AddTokenValues_BlockTokens(var ATokenValues: TEventLogTokenValues;
  APBlock: Pointer; APBuffer, APBufferEnd: PWideChar): PWideChar;
var
...
begin
  ...
  Result := AddTokenValue_NativeInt(ATokenValues, CEventLogTokenBlockSize,
    LBlockUserSize, Result, APBufferEnd);

  { Add the token for the block content type. }
  LBlockContentType := DetectBlockContentType(APBlock, LBlockUserSize);
  Result := AddTokenValue_BlockContentType(ATokenValues,
    CEventLogTokenObjectClass, LBlockContentType, Result, APBufferEnd);

  { Add the token for the block adddress in hex. }
  Result := AddTokenValue_Hexadecimal(ATokenValues, CEventLogTokenBlockAddress,
    NativeUInt(APBlock), Result, APBufferEnd);
  ...
  Result := AddTokenValue_NativeInt(ATokenValues, CEventLogTokenMemoryDumpSize,
    LMemoryDumpSize, Result, APBufferEnd);

  Result := AddTokenValue_HexDump(ATokenValues, CEventLogTokenHexDump, APBlock,
    LMemoryDumpSize, Result, APBufferEnd);

  Result := AddTokenValue_ASCIIDump(ATokenValues, CEventLogTokenASCIIDump,
    APBlock, LMemoryDumpSize, Result, APBufferEnd);

  { If this is a debug sub-block, log the additional debug information. }
  LBlockHeader := PBlockStatusFlags(APBlock)[-1];
  if LBlockHeader and (CIsSmallBlockFlag or CIsMediumBlockFlag or
    CIsLargeBlockFlag or CIsDebugBlockFlag) = CIsDebugBlockFlag then
  begin
    LPDebugBlockHeader := @PFastMM_DebugBlockHeader(APBlock)[-1];

    Result := AddTokenValue_Hexadecimal(ATokenValues,
      CEventLogTokenAllocatedByThread, LPDebugBlockHeader.AllocatedByThread,
      Result, APBufferEnd);

    Result := AddTokenValue_NativeUInt(ATokenValues,
      CEventLogTokenAllocationNumber, LPDebugBlockHeader.AllocationNumber,
      Result, APBufferEnd);

    Result := AddTokenValue_StackTrace(ATokenValues,
      CEventLogTokenAllocationStackTrace,
      LPDebugBlockHeader.DebugFooter_AllocationStackTracePtr,
      LPDebugBlockHeader.StackTraceEntryCount, Result, APBufferEnd);

    if LBlockHeader and CBlockIsFreeFlag = CBlockIsFreeFlag then
    begin
      Result := AddTokenValue_Hexadecimal(ATokenValues,
        CEventLogTokenFreedByThread, LPDebugBlockHeader.FreedByThread, Result,
        APBufferEnd);

      Result := AddTokenValue_StackTrace(ATokenValues,
        CEventLogTokenFreeStackTrace,
        LPDebugBlockHeader.DebugFooter_FreeStackTracePtr,
        LPDebugBlockHeader.StackTraceEntryCount, Result, APBufferEnd);

      { If it is a freed debug block then get the prior class from the debug header. }
      LBlockContentType :=
        NativeUInt(FastMM_DetectClassInstance
        (@LPDebugBlockHeader.PreviouslyUsedByClass));
      Result := AddTokenValue_BlockContentType(ATokenValues,
        CEventLogTokenObjectClass, LBlockContentType, Result, APBufferEnd);
...
function SubstituteTokenValues(APTemplate: PWideChar;
  const ATokenValues: TEventLogTokenValues; APBuffer, APBufferEnd: PWideChar)
  : PWideChar;
const
  CTokenStartChar = '{';
  CTokenEndChar = '}';
...
begin
  Result := EventLogFileIsOpen or OpenOrCreateTextFile(@EventLogFilename, True,
    EventLogFileHandle);
end;
...
procedure LogEvent(AEventType: TFastMM_MemoryManagerEventType;
  const ATokenValues: TEventLogTokenValues);
var
...
begin
  ...
  if AEventType in FastMM_LogToFileEvents then
    LPBuffer := SubstituteTokenValues(FastMM_LogFileEntryHeader, ATokenValues,
      LPBuffer, LPBufferEnd);
  ...
  LPBuffer := SubstituteTokenValues(LPTextTemplate, ATokenValues, LPBuffer,
    LPBufferEnd);
  ...
  begin
    ...
    if LEventLogFileWasOpen or OpenEventLogFile then
      AppendTextFile(EventLogFileHandle, LPLogHeaderStart,
        CharCount(LPBuffer, @LTextBuffer));
...
function TFastMM_FreedObject.SafeCallException(ExceptObject: TObject;
...
procedure TFastMM_FreedObject.VirtualMethodOnFreedObject
  (APMethodName: PWideChar);
begin
  { Get the stack trace and then log the event. }
  FastMM_GetStackTrace(@FVirtualMethodStackTrace,
    length(FVirtualMethodStackTrace), 0);
...
begin
  LTextBuffer[0] := '#';
  LPEnd := NativeUIntToTextBuffer(AIndex, @LTextBuffer[1],
    @LTextBuffer[High(LTextBuffer)]);
  LPEnd^ := #0;
  VirtualMethodOnFreedObject(@LTextBuffer);
end;

procedure TFastMM_FreedObject.VirtualMethodOnFreedObject_LogEvent
  (APMethodName: PWideChar);
var
...
begin
  ...
  LPBufferPos := AddTokenValues_GeneralTokens(LTokenValues, @LTokenValueBuffer,
    LPBufferEnd);
  LPBufferPos := AddTokenValues_BlockTokens(LTokenValues, Pointer(Self),
    LPBufferPos, LPBufferEnd);
  LPBufferPos := AddTokenValue(LTokenValues, CEventLogTokenVirtualMethodName,
    APMethodName, GetStringLength(APMethodName), LPBufferPos, LPBufferEnd);
  AddTokenValue_StackTrace(LTokenValues,
    CEventLogTokenVirtualMethodCallOnFreedObject,
    @TFastMM_FreedObject.FVirtualMethodStackTrace,
    length(FVirtualMethodStackTrace), LPBufferPos, LPBufferEnd);
...
procedure TFastMM_FreedObject.VirtualMethod0;
begin
  VirtualMethodOnFreedObject(0);
end;

procedure TFastMM_FreedObject.VirtualMethod1;
begin
  VirtualMethodOnFreedObject(1);
end;

procedure TFastMM_FreedObject.VirtualMethod2;
begin
  VirtualMethodOnFreedObject(2);
end;

procedure TFastMM_FreedObject.VirtualMethod3;
begin
  VirtualMethodOnFreedObject(3);
end;

procedure TFastMM_FreedObject.VirtualMethod4;
begin
  VirtualMethodOnFreedObject(4);
end;

procedure TFastMM_FreedObject.VirtualMethod5;
begin
  VirtualMethodOnFreedObject(5);
end;

procedure TFastMM_FreedObject.VirtualMethod6;
begin
  VirtualMethodOnFreedObject(6);
end;

procedure TFastMM_FreedObject.VirtualMethod7;
begin
  VirtualMethodOnFreedObject(7);
end;

procedure TFastMM_FreedObject.VirtualMethod8;
begin
  VirtualMethodOnFreedObject(8);
end;

procedure TFastMM_FreedObject.VirtualMethod9;
begin
  VirtualMethodOnFreedObject(9);
end;

procedure TFastMM_FreedObject.VirtualMethod10;
begin
  VirtualMethodOnFreedObject(10);
end;

procedure TFastMM_FreedObject.VirtualMethod11;
begin
  VirtualMethodOnFreedObject(11);
end;

procedure TFastMM_FreedObject.VirtualMethod12;
begin
  VirtualMethodOnFreedObject(12);
end;

procedure TFastMM_FreedObject.VirtualMethod13;
begin
  VirtualMethodOnFreedObject(13);
end;

procedure TFastMM_FreedObject.VirtualMethod14;
begin
  VirtualMethodOnFreedObject(14);
end;

procedure TFastMM_FreedObject.VirtualMethod15;
begin
  VirtualMethodOnFreedObject(15);
end;

procedure TFastMM_FreedObject.VirtualMethod16;
begin
  VirtualMethodOnFreedObject(16);
end;

procedure TFastMM_FreedObject.VirtualMethod17;
begin
  VirtualMethodOnFreedObject(17);
end;

procedure TFastMM_FreedObject.VirtualMethod18;
begin
  VirtualMethodOnFreedObject(18);
end;

procedure TFastMM_FreedObject.VirtualMethod19;
begin
  VirtualMethodOnFreedObject(19);
end;

procedure TFastMM_FreedObject.VirtualMethod20;
begin
  VirtualMethodOnFreedObject(20);
end;

procedure TFastMM_FreedObject.VirtualMethod21;
begin
  VirtualMethodOnFreedObject(21);
end;

procedure TFastMM_FreedObject.VirtualMethod22;
begin
  VirtualMethodOnFreedObject(22);
end;

procedure TFastMM_FreedObject.VirtualMethod23;
begin
  VirtualMethodOnFreedObject(23);
end;

procedure TFastMM_FreedObject.VirtualMethod24;
begin
  VirtualMethodOnFreedObject(24);
end;

procedure TFastMM_FreedObject.VirtualMethod25;
begin
  VirtualMethodOnFreedObject(25);
end;

procedure TFastMM_FreedObject.VirtualMethod26;
begin
  VirtualMethodOnFreedObject(26);
end;

procedure TFastMM_FreedObject.VirtualMethod27;
begin
  VirtualMethodOnFreedObject(27);
end;

procedure TFastMM_FreedObject.VirtualMethod28;
begin
  VirtualMethodOnFreedObject(28);
end;

procedure TFastMM_FreedObject.VirtualMethod29;
begin
  VirtualMethodOnFreedObject(29);
end;

procedure TFastMM_FreedObject.VirtualMethod30;
begin
  VirtualMethodOnFreedObject(30);
end;

procedure TFastMM_FreedObject.VirtualMethod31;
begin
  VirtualMethodOnFreedObject(31);
end;

procedure TFastMM_FreedObject.VirtualMethod32;
begin
  VirtualMethodOnFreedObject(32);
end;

procedure TFastMM_FreedObject.VirtualMethod33;
begin
  VirtualMethodOnFreedObject(33);
end;

procedure TFastMM_FreedObject.VirtualMethod34;
begin
  VirtualMethodOnFreedObject(34);
end;

procedure TFastMM_FreedObject.VirtualMethod35;
begin
  VirtualMethodOnFreedObject(35);
end;

procedure TFastMM_FreedObject.VirtualMethod36;
begin
  VirtualMethodOnFreedObject(36);
end;

procedure TFastMM_FreedObject.VirtualMethod37;
begin
  VirtualMethodOnFreedObject(37);
end;

procedure TFastMM_FreedObject.VirtualMethod38;
begin
  VirtualMethodOnFreedObject(38);
end;

procedure TFastMM_FreedObject.VirtualMethod39;
begin
  VirtualMethodOnFreedObject(39);
end;

procedure TFastMM_FreedObject.VirtualMethod40;
begin
  VirtualMethodOnFreedObject(40);
end;

procedure TFastMM_FreedObject.VirtualMethod41;
begin
  VirtualMethodOnFreedObject(41);
end;

procedure TFastMM_FreedObject.VirtualMethod42;
begin
  VirtualMethodOnFreedObject(42);
end;

procedure TFastMM_FreedObject.VirtualMethod43;
begin
  VirtualMethodOnFreedObject(43);
end;

procedure TFastMM_FreedObject.VirtualMethod44;
begin
  VirtualMethodOnFreedObject(44);
end;

procedure TFastMM_FreedObject.VirtualMethod45;
begin
  VirtualMethodOnFreedObject(45);
end;

procedure TFastMM_FreedObject.VirtualMethod46;
begin
  VirtualMethodOnFreedObject(46);
end;

procedure TFastMM_FreedObject.VirtualMethod47;
begin
  VirtualMethodOnFreedObject(47);
end;

procedure TFastMM_FreedObject.VirtualMethod48;
begin
  VirtualMethodOnFreedObject(48);
end;

procedure TFastMM_FreedObject.VirtualMethod49;
begin
  VirtualMethodOnFreedObject(49);
end;

procedure TFastMM_FreedObject.VirtualMethod50;
begin
  VirtualMethodOnFreedObject(50);
end;

procedure TFastMM_FreedObject.VirtualMethod51;
begin
  VirtualMethodOnFreedObject(51);
end;

procedure TFastMM_FreedObject.VirtualMethod52;
begin
  VirtualMethodOnFreedObject(52);
end;

procedure TFastMM_FreedObject.VirtualMethod53;
begin
  VirtualMethodOnFreedObject(53);
end;

procedure TFastMM_FreedObject.VirtualMethod54;
begin
  VirtualMethodOnFreedObject(54);
end;

procedure TFastMM_FreedObject.VirtualMethod55;
begin
  VirtualMethodOnFreedObject(55);
end;

procedure TFastMM_FreedObject.VirtualMethod56;
begin
  VirtualMethodOnFreedObject(56);
end;

procedure TFastMM_FreedObject.VirtualMethod57;
begin
  VirtualMethodOnFreedObject(57);
end;

procedure TFastMM_FreedObject.VirtualMethod58;
begin
  VirtualMethodOnFreedObject(58);
end;

procedure TFastMM_FreedObject.VirtualMethod59;
begin
  VirtualMethodOnFreedObject(59);
end;

procedure TFastMM_FreedObject.VirtualMethod60;
begin
  VirtualMethodOnFreedObject(60);
end;

procedure TFastMM_FreedObject.VirtualMethod61;
begin
  VirtualMethodOnFreedObject(61);
end;

procedure TFastMM_FreedObject.VirtualMethod62;
begin
  VirtualMethodOnFreedObject(62);
end;

procedure TFastMM_FreedObject.VirtualMethod63;
begin
  VirtualMethodOnFreedObject(63);
end;

procedure TFastMM_FreedObject.VirtualMethod64;
begin
  VirtualMethodOnFreedObject(64);
end;

procedure TFastMM_FreedObject.VirtualMethod65;
begin
  VirtualMethodOnFreedObject(65);
end;

procedure TFastMM_FreedObject.VirtualMethod66;
begin
  VirtualMethodOnFreedObject(66);
end;

procedure TFastMM_FreedObject.VirtualMethod67;
begin
  VirtualMethodOnFreedObject(67);
end;

procedure TFastMM_FreedObject.VirtualMethod68;
begin
  VirtualMethodOnFreedObject(68);
end;

procedure TFastMM_FreedObject.VirtualMethod69;
begin
  VirtualMethodOnFreedObject(69);
end;

procedure TFastMM_FreedObject.VirtualMethod70;
begin
  VirtualMethodOnFreedObject(70);
end;

procedure TFastMM_FreedObject.VirtualMethod71;
begin
  VirtualMethodOnFreedObject(71);
end;

procedure TFastMM_FreedObject.VirtualMethod72;
begin
  VirtualMethodOnFreedObject(72);
end;

procedure TFastMM_FreedObject.VirtualMethod73;
begin
  VirtualMethodOnFreedObject(73);
end;

procedure TFastMM_FreedObject.VirtualMethod74;
begin
  VirtualMethodOnFreedObject(74);
end;
...
begin
  Result := PBlockStatusFlags(APSmallMediumOrLargeBlock)[-1] and
    CBlockIsFreeFlag <> 0;
end;

{ Tags a block as free, without affecting any other flags. }
procedure SetBlockIsFreeFlag(APSmallMediumOrLargeBlock: Pointer;
  ABlockIsFree: Boolean); inline;
begin
  if ABlockIsFree then
    PBlockStatusFlags(APSmallMediumOrLargeBlock)[-1] :=
      PBlockStatusFlags(APSmallMediumOrLargeBlock)[-1] or CBlockIsFreeFlag
  else
    PBlockStatusFlags(APSmallMediumOrLargeBlock)[-1] :=
      PBlockStatusFlags(APSmallMediumOrLargeBlock)[-1] and
      (not CBlockIsFreeFlag);
end;
...
procedure SetBlockHasDebugInfo(APSmallMediumOrLargeBlock: Pointer;
  ABlockHasDebugInfo: Boolean); inline;
begin
  if ABlockHasDebugInfo then
    PBlockStatusFlags(APSmallMediumOrLargeBlock)[-1] :=
      PBlockStatusFlags(APSmallMediumOrLargeBlock)[-1] or CHasDebugInfoFlag
  else
    PBlockStatusFlags(APSmallMediumOrLargeBlock)[-1] :=
      PBlockStatusFlags(APSmallMediumOrLargeBlock)[-1] and
      (not CHasDebugInfoFlag);
end;

{ Calculates the size of a debug block footer, given the number of stack trace entries. }
function CalculateDebugBlockFooterSize(AStackTraceDepth: Byte)
  : NativeInt; inline;
begin
  { The debug footer consists of a dword checksum, followed by the allocation and free stack traces. }
  Result := CDebugBlockFooterCheckSumSize + AStackTraceDepth *
    (2 * SizeOf(Pointer));
end;

procedure LogDebugBlockHeaderInvalid(APDebugBlockHeader
  : PFastMM_DebugBlockHeader);
var
  LTokenValues: TEventLogTokenValues;
...
begin
  ...
  LPBufferPos := AddTokenValues_GeneralTokens(LTokenValues, @LTokenValueBuffer,
    LPBufferEnd);
  AddTokenValues_BlockTokens(LTokenValues, APDebugBlockHeader, LPBufferPos,
    LPBufferEnd);
...
procedure LogDebugBlockFooterInvalid(APDebugBlockHeader
  : PFastMM_DebugBlockHeader);
var
  LTokenValues: TEventLogTokenValues;
  LTokenValueBuffer: array [0 .. CTokenBufferMaxWideChars - 1] of WideChar;
...
begin
  ...
  LPBufferPos := AddTokenValues_GeneralTokens(LTokenValues, @LTokenValueBuffer,
    LPBufferEnd);
  AddTokenValues_BlockTokens(LTokenValues, PByte(APDebugBlockHeader) +
    CDebugBlockHeaderSize, LPBufferPos, LPBufferEnd);
...
function CheckDebugBlockHeaderAndFooterCheckSumsValid(APDebugBlockHeader
  : PFastMM_DebugBlockHeader): Boolean;
begin
  if APDebugBlockHeader.CalculateHeaderCheckSum <> APDebugBlockHeader.HeaderCheckSum
  then
  begin
    LogDebugBlockHeaderInvalid(APDebugBlockHeader);
    Exit(False);
  end;
  if APDebugBlockHeader.CalculateFooterCheckSum <>
    APDebugBlockHeader.DebugFooterPtr^ then
  begin
...
procedure FillDebugBlockWithDebugPattern(APDebugBlockHeader
  : PFastMM_DebugBlockHeader);
var
...
procedure LogDebugBlockFillPatternCorrupted(APDebugBlockHeader
  : PFastMM_DebugBlockHeader);
const
...
begin
  ...
  begin
    ...
    begin
      ...
      begin
        Inc(LOffset);
        if (LOffset >= APDebugBlockHeader.UserSize) or
          (LPUserArea[LOffset] = CDebugFillPattern1B) then
        begin
          Break;
        end;
      end;
      ...
      LPBufferPos := NativeIntToTextBuffer(LChangeStart, LPBufferPos,
        LPBufferEnd);
      LPBufferPos^ := '(';
      Inc(LPBufferPos);
      LPBufferPos := NativeIntToTextBuffer(LOffset - LChangeStart, LPBufferPos,
        LPBufferEnd);
      LPBufferPos^ := ')';
      Inc(LPBufferPos);
  ...
  LPBufferPos := AddTokenValues_GeneralTokens(LTokenValues, LPBufferPos,
    LPBufferEnd);
  AddTokenValues_BlockTokens(LTokenValues, PByte(APDebugBlockHeader) +
    CDebugBlockHeaderSize, LPBufferPos, LPBufferEnd);
...
{ Checks that the debug fill pattern in the debug block is intact.  Returns True if the block is intact, otherwise
  (optionally) logs and/or displays the error and returns False. }
function CheckDebugBlockFillPatternIntact(APDebugBlockHeader
  : PFastMM_DebugBlockHeader): Boolean;
var
  LByteOffset: NativeInt;
  LPUserArea: PByte;
...
function CheckFreeDebugBlockIntact(APDebugBlockHeader
  : PFastMM_DebugBlockHeader): Boolean;
begin
  Result := CheckDebugBlockHeaderAndFooterCheckSumsValid(APDebugBlockHeader) and
    CheckDebugBlockFillPatternIntact(APDebugBlockHeader);
end;
...
{$IFDEF 32Bit}
  if EmergencyReserveAddressSpace = nil then
    EmergencyReserveAddressSpace := OS_AllocateVirtualMemory
      (CEmergencyReserveAddressSpace, True);
{$ENDIF}
end;
...
begin
  ...
  if LPActualBlock.StackTraceEntryCount > 0 then
    FastMM_GetStackTrace(LPActualBlock.DebugFooter_FreeStackTracePtr,
      LPActualBlock.StackTraceEntryCount,
      CFastMM_StackTrace_SkipFrames_FreeMem);
...
function FastMM_ReallocMem_ReallocDebugBlock(APointer: Pointer;
  ANewSize: NativeInt): Pointer;
var
  LPActualBlock: PFastMM_DebugBlockHeader;
...
begin
  ...
  LDebugFooterSize := CalculateDebugBlockFooterSize
    (LPActualBlock.StackTraceEntryCount);
  if LAvailableSpace >= (ANewSize + CDebugBlockHeaderSize + LDebugFooterSize)
  then
  begin
  ...
  begin
    ...
    Result := FastMM_GetMem(ANewSize + CDebugBlockHeaderSize +
      LDebugFooterSize);

    if Result <> nil then
    begin
      { Move the old header and data across }
      System.Move(LPActualBlock^, Result^, LPActualBlock.UserSize +
      ...
      PFastMM_DebugBlockHeader(Result).HeaderCheckSum :=
        PFastMM_DebugBlockHeader(Result).CalculateHeaderCheckSum;
...
{ Always returns - 1. }
function HandleInvalidFreeMemOrReallocMem(APointer: Pointer;
  AIsReallocMemCall: Boolean): Integer;
var
  LPDebugBlockHeader: PFastMM_DebugBlockHeader;
...
begin
  ...
  if PBlockStatusFlags(APointer)[-1] <> (CBlockIsFreeFlag or CIsDebugBlockFlag)
  then
    Exit(-1);
...
function FastMM_FreeMem_FreeLargeBlock_ReleaseVM(APLargeBlockHeader
  : PLargeBlockHeader): Integer;
var
...
begin
  ...
  begin
    Result := OS_FreeVirtualMemory(APLargeBlockHeader,
      APLargeBlockHeader.ActualBlockSize);
  end
  else
  begin
...
{$ENDIF}
    ...
    begin
      ...
      Result := OS_FreeVirtualMemory(LPCurrentSegment,
        NativeInt(LMemoryRegionInfo.RegionSize));
      if Result <> 0 then
        Break;
...
function ProcessLargeBlockPendingFrees_ArenaAlreadyLocked(APLargeBlockManager
  : PLargeBlockManager): Integer;
var
...
begin
  ...
  LOldPendingFreeList := AtomicExchange
    (APLargeBlockManager.PendingFreeList, nil);
...
begin
  ...
  begin
    ...
    if (LPLargeBlockManager.PendingFreeList <> nil) and
      (LPLargeBlockManager.LargeBlockManagerLocked = 0) and
      (AtomicExchange(LPLargeBlockManager.LargeBlockManagerLocked, 1) = 0) then
    begin

      Result := ProcessLargeBlockPendingFrees_ArenaAlreadyLocked
        (LPLargeBlockManager);
...
begin
  ...
  LLargeBlockActualSize :=
    (ASize + CLargeBlockHeaderSize + CLargeBlockGranularity - 1) and
    -CLargeBlockGranularity;
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        if (LPLargeBlockManager.LargeBlockManagerLocked = 0) and
          (AtomicExchange(LPLargeBlockManager.LargeBlockManagerLocked, 1) = 0)
        then
        begin
          ...
          PLargeBlockHeader(Result).PreviousLargeBlockHeader :=
            Pointer(LPLargeBlockManager);
...
begin
  ...
  if AtomicCmpExchange(LPLargeBlockManager.LargeBlockManagerLocked, 1, 0) = 0
  then
  begin
  ...
  begin
    ...
    begin
      ...
      if AtomicCmpExchange(LPLargeBlockManager.PendingFreeList, APLargeBlock,
        LOldPendingFreeList) = LOldPendingFreeList then
        Break;
    end;
...
function FastMM_ReallocMem_ReallocLargeBlock(APointer: Pointer;
  ANewSize: NativeInt): Pointer;
var
  LPLargeBlockHeader: PLargeBlockHeader;
...
begin
  ...
  LOldAvailableSize := LPLargeBlockHeader.ActualBlockSize -
    CLargeBlockHeaderSize;
  ...
  begin
    ...
    LPNextSegment := Pointer(PByte(LPLargeBlockHeader) +
      LPLargeBlockHeader.ActualBlockSize);
    ...
    begin
      ...
      LMemoryRegionInfo.RegionSize := LMemoryRegionInfo.RegionSize and
        -CLargeBlockGranularity;
      { Enough space to grow in place? }
      if LMemoryRegionInfo.RegionSize >= NativeUInt(ANewSize - LOldAvailableSize)
      then
      begin
        { There is enough space after the block to extend it - determine by how much }
        LNewSegmentSize := (LNewAllocSize - LOldAvailableSize +
          CLargeBlockGranularity - 1) and -CLargeBlockGranularity;
        ...
        if OS_AllocateVirtualMemoryAtAddress(LPNextSegment, LNewSegmentSize,
          False) then
        begin
...
function RoundUserSizeUpToNextMediumBlockBin(AUserSize: Integer)
  : Integer; inline;
begin
  if AUserSize <= (CMediumBlockMiddleBinsStart - CMediumBlockHeaderSize) then
  begin
    Result := (AUserSize + (CMediumBlockHeaderSize - CMinimumMediumBlockSize +
      CInitialBinSpacing - 1)) and -CInitialBinSpacing +
  ...
  begin
    ...
    begin
      Result := (AUserSize + (CMediumBlockHeaderSize -
        CMediumBlockMiddleBinsStart + CMiddleBinSpacing - 1)) and
        -CMiddleBinSpacing + CMediumBlockMiddleBinsStart;
    end
    else
    begin
      Result := (AUserSize + (CMediumBlockHeaderSize -
        CMediumBlockFinalBinsStart + CFinalBinSpacing - 1)) and
        -CFinalBinSpacing + CMediumBlockFinalBinsStart;
    end;
  end;
end;
...
function GetBinNumberForMediumBlockSize(AMediumBlockSize: Integer)
  : Integer; inline;
begin
  if AMediumBlockSize <= CMediumBlockMiddleBinsStart then
  begin
    Result := (AMediumBlockSize - CMinimumMediumBlockSize)
      shr CInitialBinSpacingBits;
  end
  else
  begin
    if AMediumBlockSize <= CMediumBlockFinalBinsStart then
      Result := (AMediumBlockSize + (CInitialBinCount * CMiddleBinSpacing -
        CMediumBlockMiddleBinsStart)) shr CMiddleBinSpacingBits
    else
      Result := (AMediumBlockSize + ((CInitialBinCount + CMiddleBinCount) *
        CFinalBinSpacing - CMediumBlockFinalBinsStart))
        shr CFinalBinSpacingBits;
  end;
end;

function GetMediumBlockSpan(APMediumBlock: Pointer)
  : PMediumBlockSpanHeader; inline;
begin
  Result := PMediumBlockSpanHeader(NativeUInt(APMediumBlock) -
    (PMediumBlockHeader(APMediumBlock)[-1].MediumBlockSpanOffsetMultiple shl
    CMediumBlockAlignmentBits));
end;

function GetMediumBlockSize(APMediumBlock: Pointer): Integer; inline;
begin
  Result := PMediumBlockHeader(APMediumBlock)
    [-1].MediumBlockSizeMultiple shl CMediumBlockAlignmentBits;
end;

procedure SetMediumBlockHeader_SetIsSmallBlockSpan(APMediumBlock: Pointer;
  AIsSmallBlockSpan: Boolean); inline;
begin
  PMediumBlockHeader(APMediumBlock)[-1].IsSmallBlockSpan := AIsSmallBlockSpan;
end;

procedure SetMediumBlockHeader_SetMediumBlockSpan(APMediumBlock: Pointer;
  APMediumBlockSpan: PMediumBlockSpanHeader); inline;
begin
  { Store the offset to the medium block span. }
  PMediumBlockHeader(APMediumBlock)[-1].MediumBlockSpanOffsetMultiple :=
    (NativeUInt(APMediumBlock) - NativeUInt(APMediumBlockSpan))
    shr CMediumBlockAlignmentBits;
end;

procedure SetMediumBlockHeader_SetSizeAndFlags(APMediumBlock: Pointer;
  ABlockSize: Integer; ABlockIsFree: Boolean;
  ABlockHasDebugInfo: Boolean); inline;
var
  LPNextBlock: Pointer;
begin
  ...
  begin
    ...
    if ABlockHasDebugInfo then
      PMediumBlockHeader(APMediumBlock)[-1].BlockStatusFlags :=
        CHasDebugInfoFlag + CBlockIsFreeFlag + CIsMediumBlockFlag
    else
      PMediumBlockHeader(APMediumBlock)[-1].BlockStatusFlags := CBlockIsFreeFlag
        + CIsMediumBlockFlag;
  ...
  PMediumBlockHeader(APMediumBlock)[-1].MediumBlockSizeMultiple :=
    ABlockSize shr CMediumBlockAlignmentBits;
end;

{ Inserts a medium block into the appropriate medium block bin.  The header for APMediumFreeBlock must already be set
  correctly. }
procedure InsertMediumBlockIntoBin(APMediumBlockManager: PMediumBlockManager;
  APMediumFreeBlock: PMediumFreeBlockContent; AMediumBlockSize: Integer);
var
...
begin
  ...
  begin
    ...
    APMediumBlockManager.MediumBlockBinBitmaps[LBinGroupNumber] :=
      APMediumBlockManager.MediumBlockBinBitmaps[LBinGroupNumber] or
      (1 shl (LBinNumber and 31));
    { Flag the group as used }
    APMediumBlockManager.MediumBlockBinGroupBitmap :=
      APMediumBlockManager.MediumBlockBinGroupBitmap or (1 shl LBinGroupNumber);
  end;
end;
...
procedure RemoveMediumFreeBlockFromBin(APMediumBlockManager
  : PMediumBlockManager; APMediumFreeBlock: PMediumFreeBlockContent);
var
...
begin
  ...
  begin
    ...
    if (NativeUInt(LPNextFreeBlock) > NativeUInt(@MediumBlockManagers) +
      SizeOf(MediumBlockManagers)) or
      (NativeUInt(LPNextFreeBlock) < NativeUInt(@MediumBlockManagers)) then
    begin
  ...
  begin
    { Calculate the bin number from the bin pointer:  LPNextFreeBlock will be a pointer to the bin, since the bin is now
      empty.) }
    LBinNumber := (NativeUInt(LPNextFreeBlock) -
      NativeUInt(@APMediumBlockManager.FirstFreeBlockInBin))
      shr CPointerSizeBitShift;
    LBinGroupNumber := LBinNumber shr 5; // 32 bins per group
    { Flag this bin as empty }
    APMediumBlockManager.MediumBlockBinBitmaps[LBinGroupNumber] :=
      APMediumBlockManager.MediumBlockBinBitmaps[LBinGroupNumber] and
    ...
    begin
      ...
      APMediumBlockManager.MediumBlockBinGroupBitmap :=
        APMediumBlockManager.MediumBlockBinGroupBitmap and
...
procedure BinMediumSequentialFeedRemainder(APMediumBlockManager
  : PMediumBlockManager);
var
...
begin
  ...
  begin
    ...
    LPreviousLastSequentialFeedBlockOffset :=
      APMediumBlockManager.LastMediumBlockSequentialFeedOffset.IntegerValue;
    ...
    if AtomicCmpExchange
      (APMediumBlockManager.LastMediumBlockSequentialFeedOffset.IntegerValue, 0,
      LPreviousLastSequentialFeedBlockOffset) = LPreviousLastSequentialFeedBlockOffset
    then
    begin
      LSequentialFeedFreeSize := LPreviousLastSequentialFeedBlockOffset -
        CMediumBlockSpanHeaderSize;

      { Get the block for the remaining space }
      LPNextMediumBlock :=
        PByte(APMediumBlockManager.SequentialFeedMediumBlockSpan) +
        LPreviousLastSequentialFeedBlockOffset;

      { Point to the remainder }
      LPRemainderBlock :=
        Pointer(PByte(APMediumBlockManager.SequentialFeedMediumBlockSpan) +
        CMediumBlockSpanHeaderSize);
      ...
      { Store the size of the block as well as the flags.  Also updates the header of the next block to indicate that
        this block is free. }
      SetMediumBlockHeader_SetSizeAndFlags(LPRemainderBlock,
        LSequentialFeedFreeSize, True, False);
      SetMediumBlockHeader_SetMediumBlockSpan(LPRemainderBlock,
        APMediumBlockManager.SequentialFeedMediumBlockSpan);

      { Bin this medium block }
      if LSequentialFeedFreeSize >= CMinimumMediumBlockSize then
        InsertMediumBlockIntoBin(APMediumBlockManager, LPRemainderBlock,
...
function FastMM_FreeMem_InternalFreeMediumBlock_ManagerAlreadyLocked
  (APMediumBlockManager: PMediumBlockManager;
  APMediumBlockSpan: PMediumBlockSpanHeader; APMediumBlock: Pointer;
  AUnlockMediumBlockManager: Boolean): Integer;
var
...
begin
  ...
  begin
    ...
    begin
      LPreviousBlockSize := PMediumFreeBlockFooter(APMediumBlock)
        [-1].MediumFreeBlockSize;
    ...
    SetMediumBlockHeader_SetSizeAndFlags(APMediumBlock, LBlockSize,
      True, False);
  ...
  if (LBlockSize <> (APMediumBlockSpan.SpanSize - CMediumBlockSpanHeaderSize))
    or ((OptimizationStrategy <> mmosOptimizeForLowMemoryUsage) and
    (APMediumBlockManager.LastMediumBlockSequentialFeedOffset.IntegerValue <
    CMaximumMediumBlockSize) and (APMediumBlockManager.MediumBlockBinBitmaps
    [CMediumBlockBinGroupCount - 1] and (1 shl 31) = 0)) then
  begin
  ...
  begin
    ...
    LPPreviousMediumBlockSpan :=
      APMediumBlockSpan.PreviousMediumBlockSpanHeader;
    LPNextMediumBlockSpan := APMediumBlockSpan.NextMediumBlockSpanHeader;
    LPPreviousMediumBlockSpan.NextMediumBlockSpanHeader :=
      LPNextMediumBlockSpan;
    LPNextMediumBlockSpan.PreviousMediumBlockSpanHeader :=
      LPPreviousMediumBlockSpan;
...
{ Frees a chain of blocks belonging to the medium block manager.  The block manager is assumed to be locked.  Optionally
  ...
  block. }
function FastMM_FreeMem_FreeMediumBlockChain(APMediumBlockManager
  : PMediumBlockManager; APPendingFreeMediumBlock: Pointer;
...
begin
  ...
  begin
    ...
    Result := Result or
      FastMM_FreeMem_InternalFreeMediumBlock_ManagerAlreadyLocked
      (APMediumBlockManager, LPMediumBlockSpan, APPendingFreeMediumBlock,
      AUnlockMediumBlockManagerWhenDone and (LPNextBlock = nil));
...
begin
  ...
  if AtomicCmpExchange(LPMediumBlockManager.MediumBlockManagerLocked, 1, 0) = 0
  then
  begin
    ...
    begin
      Result := FastMM_FreeMem_InternalFreeMediumBlock_ManagerAlreadyLocked
        (LPMediumBlockManager, LPMediumBlockSpan, APMediumBlock, True);
    end
    else
    begin
      Result := FastMM_FreeMem_InternalFreeMediumBlock_ManagerAlreadyLocked
        (LPMediumBlockManager, LPMediumBlockSpan, APMediumBlock, False);

      { Process the pending frees list. }
      LFirstPendingFreeBlock :=
        AtomicExchange(LPMediumBlockManager.PendingFreeList, nil);
      Result := Result or FastMM_FreeMem_FreeMediumBlockChain
        (LPMediumBlockManager, LFirstPendingFreeBlock, True);
    end;
  ...
  begin
    ...
    begin
      ...
      if AtomicCmpExchange(LPMediumBlockManager.PendingFreeList, APMediumBlock,
        LFirstPendingFreeBlock) = LFirstPendingFreeBlock then
        Break;
    end;
...
function FastMM_GetMem_GetMediumBlock_AllocateNewSequentialFeedSpan
  (APMediumBlockManager: PMediumBlockManager; AFirstBlockSize: Integer)
  : Pointer;
var
...
begin
  ...
  begin
    ...
    LPNewSpan.PreviousMediumBlockSpanHeader :=
      PMediumBlockSpanHeader(APMediumBlockManager);
    ...
    PMediumBlockHeader(PByte(LPNewSpan) + LNewSpanSize)[-1].BlockStatusFlags :=
      CIsMediumBlockFlag;
    ...
    APMediumBlockManager.LastMediumBlockSequentialFeedOffset.IntegerValue :=
      NativeInt(Result) - NativeInt(LPNewSpan);
  end
  else
  begin
    { Out of memory }
...
function FastMM_GetMem_GetMediumBlock_TryGetBlockFromSequentialFeedSpan
  (APMediumBlockManager: PMediumBlockManager;
...
var
  ...
  LPreviousLastSequentialFeedBlockOffset, LNewLastSequentialFeedBlockOffset
    : TIntegerWithABACounter;
  LBlockSize: Integer;
begin
  ...
  begin
    LPreviousLastSequentialFeedBlockOffset :=
      APMediumBlockManager.LastMediumBlockSequentialFeedOffset;

    { Is there space available for at least the minimum size block? }
    LBlockSize := LPreviousLastSequentialFeedBlockOffset.IntegerValue -
      CMediumBlockSpanHeaderSize;
    ...
    begin
      ...
      LNewLastSequentialFeedBlockOffset.IntegerAndABACounter :=
        LPreviousLastSequentialFeedBlockOffset.IntegerAndABACounter - LBlockSize
        + (Int64(1) shl 32);

      LPSequentialFeedSpan :=
        APMediumBlockManager.SequentialFeedMediumBlockSpan;

      if AtomicCmpExchange
        (APMediumBlockManager.LastMediumBlockSequentialFeedOffset.
        IntegerAndABACounter,
        LNewLastSequentialFeedBlockOffset.IntegerAndABACounter,
        LPreviousLastSequentialFeedBlockOffset.IntegerAndABACounter)
        = LPreviousLastSequentialFeedBlockOffset.IntegerAndABACounter then
      begin
        Result := Pointer(PByte(LPSequentialFeedSpan) +
          LNewLastSequentialFeedBlockOffset.IntegerValue);
...
function FastMM_GetMem_GetMediumBlock_TryReusePendingFreeBlock
  (APMediumBlockManager: PMediumBlockManager;
...
begin
  ...
  LPPendingFreeBlock := AtomicExchange
    (APMediumBlockManager.PendingFreeList, nil);
  ...
  begin
    ...
    if (LBlockSize >= AMinimumBlockSize) and (LBlockSize < LBestMatchBlockSize)
    then
    begin
    ...
    begin
      ...
      if FastMM_FreeMem_InternalFreeMediumBlock_ManagerAlreadyLocked
        (APMediumBlockManager, LPMediumBlockSpan, LPPendingFreeBlock, False) <> 0
      then
      begin
        System.Error(reInvalidPtr);
      end;
  ...
  begin
    ...
    if BlockHasDebugInfo(Result) and (not CheckFreeDebugBlockIntact(Result))
    then
    begin
    ...
    begin
      ...
      LPSecondSplit := PMediumFreeBlockContent
        (PByte(Result) + LBestMatchBlockSize);
      LPMediumBlockSpan := GetMediumBlockSpan(Result);
      SetMediumBlockHeader_SetSizeAndFlags(LPSecondSplit, LSecondSplitSize,
        True, False);
      ...
      if LSecondSplitSize >= CMinimumMediumBlockSize then
        InsertMediumBlockIntoBin(APMediumBlockManager, LPSecondSplit,
          LSecondSplitSize);

    end;

    { Set the header and trailer for this block, clearing the debug info flag. }
    SetMediumBlockHeader_SetSizeAndFlags(Result, LBestMatchBlockSize,
      False, False);
...
function FastMM_GetMem_GetMediumBlock_AllocateFreeBlockAndUnlockArena
  (APMediumBlockManager: PMediumBlockManager; AMinimumBlockSizeBinNumber,
  AOptimalBlockSize, AMaximumBlockSize: Integer): Pointer;
{$IFNDEF PurePascal}
const
  ...
  @SecondSplitDone:
    ...
    {$ELSE}
var
  LBinGroupNumber, LBinNumber, LBinGroupMasked, LBinGroupsMasked, LBlockSize,
    LSecondSplitSize: Integer;
  LPSecondSplit: PMediumFreeBlockContent;
begin
  ...
  LBinGroupMasked := APMediumBlockManager.MediumBlockBinBitmaps[LBinGroupNumber]
    and ((-1) shl (AMinimumBlockSizeBinNumber and 31));
  if LBinGroupMasked <> 0 then
  begin
    { There is a block in the group containing AMinimumBlockSizeBinNumber, get the exact bin number. }
    LBinNumber := CountTrailingZeros32(LBinGroupMasked) +
      (LBinGroupNumber shl 5);
  end
  else
  begin
    { There are no suitable free blocks in the group containing AMinimumBlockSizeBinNumber, so get a free block from any
      subsequent group. }
    LBinGroupsMasked := APMediumBlockManager.MediumBlockBinGroupBitmap and
      ((-2) shl LBinGroupNumber);
    ...
    LBinNumber := CountTrailingZeros32
      (APMediumBlockManager.MediumBlockBinBitmaps[LBinGroupNumber]) +
      (LBinGroupNumber shl 5);
  end;
  ...
  { If the block currently has debug info, check it for consistency before resetting the flag. }
  if BlockHasDebugInfo(Result) and (not CheckFreeDebugBlockIntact(Result)) then
  begin
    APMediumBlockManager.MediumBlockManagerLocked := 0;
    System.Error(reInvalidPtr);
  end;
  ...
  begin
    ...
    SetMediumBlockHeader_SetSizeAndFlags(LPSecondSplit, LSecondSplitSize,
      True, False);
    SetMediumBlockHeader_SetMediumBlockSpan(LPSecondSplit,
      GetMediumBlockSpan(Result));
...
function FastMM_GetMem_GetMediumBlock(AMinimumBlockSize, AOptimalBlockSize,
  AMaximumBlockSize: Integer): Pointer;
{$IFDEF X86ASM}
const
...
var
  LPMediumBlockManager: PMediumBlockManager;
  LMinimumBlockSizeBinNumber, LMinimumBlockSizeBinGroupNumber,
    LMinimumBlockSizeBinMask, LLargerBinGroupsMask: Integer;
begin
  ...
  LMinimumBlockSizeBinNumber := GetBinNumberForMediumBlockSize
    (AMinimumBlockSize);
  LMinimumBlockSizeBinGroupNumber := LMinimumBlockSizeBinNumber shr 5;
  // 32 bins per group
  ...
  begin
    ...
    begin
      ...
      { In order to process the pending free lists or get a block from the free lists the block manager must be locked.
        ...
        allocation. }
      if (LPMediumBlockManager.MediumBlockManagerLocked = 0) and
        ((LPMediumBlockManager.PendingFreeList <> nil) or
        ((LPMediumBlockManager.MediumBlockBinGroupBitmap and
        LLargerBinGroupsMask) <> 0) or
        ((LPMediumBlockManager.MediumBlockBinBitmaps
        [LMinimumBlockSizeBinGroupNumber] and LMinimumBlockSizeBinMask) <> 0))
        and (AtomicExchange(LPMediumBlockManager.MediumBlockManagerLocked,
        1) = 0) then
      begin
        ...
        if LPMediumBlockManager.PendingFreeList <> nil then
        begin
          Result := FastMM_GetMem_GetMediumBlock_TryReusePendingFreeBlock
            (LPMediumBlockManager, AMinimumBlockSize, AOptimalBlockSize,
            AMaximumBlockSize);
          if Result <> nil then
          begin
        ...
        { 1.2) Try to find a suitable free block in the free lists }
        if ((LPMediumBlockManager.MediumBlockBinGroupBitmap and
          LLargerBinGroupsMask) <> 0) or
          ((LPMediumBlockManager.MediumBlockBinBitmaps
          [LMinimumBlockSizeBinGroupNumber] and LMinimumBlockSizeBinMask) <> 0)
        then
        begin
          Exit(FastMM_GetMem_GetMediumBlock_AllocateFreeBlockAndUnlockArena
            (LPMediumBlockManager, LMinimumBlockSizeBinNumber,
      ...
      { Try the next arena. }
      if NativeUInt(LPMediumBlockManager) >=
        NativeUInt(@MediumBlockManagers[CFastMM_MediumBlockArenaCount - 1]) then
        Break;
    ...
    begin
      ...
      { 2.1) Try to feed a medium block sequentially from an existing sequential feed span }
      if LPMediumBlockManager.LastMediumBlockSequentialFeedOffset.IntegerValue
        >= (AMinimumBlockSize + CMediumBlockSpanHeaderSize) then
      begin
        Result := FastMM_GetMem_GetMediumBlock_TryGetBlockFromSequentialFeedSpan
          (LPMediumBlockManager, AMinimumBlockSize, AOptimalBlockSize);
        if Result <> nil then
          Exit;
      end;
    ...
    begin
      ...
      if AtomicCmpExchange(LPMediumBlockManager.MediumBlockManagerLocked, 1,
        0) = 0 then
      begin
        ...
        Result := FastMM_GetMem_GetMediumBlock_AllocateNewSequentialFeedSpan
          (LPMediumBlockManager, AOptimalBlockSize);
        LPMediumBlockManager.MediumBlockManagerLocked := 0;
        Exit;
      end;
...
function FastMM_ReallocMem_ReallocMediumBlock_Upsize(APointer: Pointer;
  ANewUserSize: NativeInt): Pointer;
var
  LPNextBlock: Pointer;
  LBlockSize, LOldUserSize, LNextBlockSize, LCombinedUserSize, LNewAllocSize,
    LNewBlockSize, LSecondSplitSize: NativeInt;
  LPMediumBlockSpan: PMediumBlockSpanHeader;
  LPMediumBlockManager: PMediumBlockManager;
begin
  ...
  begin
    ...
    begin
      ...
      LPMediumBlockManager := LPMediumBlockSpan.MediumBlockManager;
      if (LPMediumBlockManager.MediumBlockManagerLocked = 0) and
        (AtomicExchange(LPMediumBlockManager.MediumBlockManagerLocked, 1) = 0)
      then
      begin
        ...
        if (ANewUserSize <= LCombinedUserSize) and BlockIsFree(LPNextBlock) then
        begin
          if LNextBlockSize >= CMinimumMediumBlockSize then
          ...
          LNewBlockSize :=
            ((LNewAllocSize + (CMediumBlockHeaderSize + CMediumBlockAlignment -
            1)) and -CMediumBlockAlignment);
          { Calculate the size of the second split }
          LSecondSplitSize := LCombinedUserSize + CMediumBlockHeaderSize -
            LNewBlockSize;
          { Does it fit? }
          ...
          begin
            { Split the block in two }
            LPNextBlock := PMediumFreeBlockContent
              (PByte(APointer) + LNewBlockSize);

            SetMediumBlockHeader_SetSizeAndFlags(LPNextBlock, LSecondSplitSize,
              True, False);
            SetMediumBlockHeader_SetMediumBlockSpan(LPNextBlock,
              LPMediumBlockSpan);
            { The second split is an entirely new block so all the header fields must be set. }
            ...
            if LSecondSplitSize >= CMinimumMediumBlockSize then
              InsertMediumBlockIntoBin(LPMediumBlockManager, LPNextBlock,
                LSecondSplitSize);
          end;

          { Set the size and flags for this block }
          SetMediumBlockHeader_SetSizeAndFlags(APointer, LNewBlockSize,
            False, False);
...
function FastMM_ReallocMem_ReallocMediumBlock_Downsize(APointer: Pointer;
  ANewUserSize: NativeInt): Pointer;
var
  LPNextBlock: Pointer;
...
begin
  ...
  begin
    ...
    LNewBlockSize :=
      ((ANewUserSize + (CMediumBlockHeaderSize + CMediumBlockAlignment - 1)) and
      -CMediumBlockAlignment);
...
function FastMM_ReallocMem_ReallocMediumBlock(APointer: Pointer;
  ANewUserSize: NativeInt): Pointer;
{$IFDEF X86ASM}
asm
...
begin
  ...
  begin
    Result := FastMM_ReallocMem_ReallocMediumBlock_Upsize(APointer,
      ANewUserSize);
  end
  else
  begin
    ...
    if (LOldUserSize shr 1) > ANewUserSize then
      Result := FastMM_ReallocMem_ReallocMediumBlock_Downsize(APointer,
        ANewUserSize)
    else
      Result := APointer;
  end;
...
procedure SetSmallBlockHeader(APSmallBlock: Pointer;
  APSmallBlockSpan: PSmallBlockSpanHeader; ABlockIsFree: Boolean;
  ABlockHasDebugInfo: Boolean); inline;
begin
  if ABlockIsFree then
  begin
    ...
    begin
      PSmallBlockHeader(APSmallBlock)[-1].BlockStatusFlagsAndSpanOffset :=
        (((NativeInt(APSmallBlock) - NativeInt(APSmallBlockSpan)) and
        -CMediumBlockAlignment) shr CSmallBlockSpanOffsetBitShift) +
...
function GetSpanForSmallBlock(APSmallBlock: Pointer)
  : PSmallBlockSpanHeader; inline;
begin
  Result := Pointer((NativeInt(APSmallBlock) and -CMediumBlockAlignment) -
    (CDropSmallBlockFlagsMask and PBlockStatusFlags(APSmallBlock)[-1])
    shl CSmallBlockSpanOffsetBitShift);
end;
...
function FastMM_FreeMem_FreeSmallBlock_ManagerAlreadyLocked(APSmallBlockSpan
  : PSmallBlockSpanHeader; APSmallBlock: Pointer;
  AUnlockSmallBlockManager: Boolean): Integer;
{$IFDEF X86ASM}
asm
  push ebx
...
begin
  ...
  begin
    ...
    begin
      ...
      APSmallBlockSpan.PreviousPartiallyFreeSpan :=
        PSmallBlockSpanHeader(LPSmallBlockManager);
...
begin
  ...
  begin
    ...
    FastMM_FreeMem_FreeSmallBlock_ManagerAlreadyLocked(LPSmallBlockSpan,
      APPendingFreeSmallBlock, AUnlockSmallBlockManagerWhenDone and
      (LPNextBlock = nil));
...
begin
  ...
  if AtomicCmpExchange(LPSmallBlockManager.SmallBlockManagerLocked, 1, 0) = 0
  then
  begin
    ...
    begin
      Result := FastMM_FreeMem_FreeSmallBlock_ManagerAlreadyLocked
        (LPSmallBlockSpan, APSmallBlock, True);
      Exit;
    end
    else
    begin
      ...
      { Process the pending frees list. }
      LFirstPendingFreeBlock :=
        AtomicExchange(LPSmallBlockManager.PendingFreeList, nil);
      Result := FastMM_FreeMem_FreeSmallBlockChain
        (LFirstPendingFreeBlock, True);
    end;
  ...
  begin
    ...
    begin
      ...
      PPointer(APSmallBlock)^ := LOldFirstFreeBlock;
      if AtomicCmpExchange(LPSmallBlockManager.PendingFreeList, APSmallBlock,
        LOldFirstFreeBlock) = LOldFirstFreeBlock then
        Break;
    end;
    Result := 0;
  end;
...
function FastMM_GetMem_GetSmallBlock_AllocateNewSequentialFeedSpanAndUnlockArena
...
begin
  LPSmallBlockSpan := FastMM_GetMem_GetMediumBlock
    (APSmallBlockManager.MinimumSpanSize, APSmallBlockManager.OptimalSpanSize,
    APSmallBlockManager.OptimalSpanSize +
    CSmallBlockSpanMaximumAmountWithWhichOptimalSizeMayBeExceeded);
  ...
  { Calculate the number of small blocks that will fit inside the span.  We need to account for the span header, as well
    ...
    fed sequentially. }
  LTotalBlocksInSpan :=
    (LSpanSize - (CSmallBlockSpanHeaderSize + CMediumBlockHeaderSize -
    CSmallBlockHeaderSize)) div APSmallBlockManager.BlockSize;
  ...
  LLastBlockOffset := CSmallBlockSpanHeaderSize + APSmallBlockManager.BlockSize
    * (LTotalBlocksInSpan - 1);
  APSmallBlockManager.LastSmallBlockSequentialFeedOffset.IntegerValue :=
    LLastBlockOffset;
...
function FastMM_GetMem_GetSmallBlock_TryGetBlockFromSequentialFeedSpan
  (APSmallBlockManager: PSmallBlockManager): Pointer;
{$IFNDEF PurePascal}
asm
  {$IFDEF X86ASM}
...
begin
  ...
  begin
    LPreviousLastSequentialFeedBlockOffset :=
      APSmallBlockManager.LastSmallBlockSequentialFeedOffset;
    ...
    if LPreviousLastSequentialFeedBlockOffset.IntegerValue <= CSmallBlockSpanHeaderSize
    then
      Exit(nil);

    if AtomicCmpExchange(APSmallBlockManager.LastSmallBlockSequentialFeedOffset.
      IntegerAndABACounter,
      LNewLastSequentialFeedBlockOffset.IntegerAndABACounter,
      LPreviousLastSequentialFeedBlockOffset.IntegerAndABACounter)
      = LPreviousLastSequentialFeedBlockOffset.IntegerAndABACounter then
    begin
      ...
      Result := @PByte(LPSequentialFeedSpan)
        [LNewLastSequentialFeedBlockOffset.IntegerValue];
...
begin
  ...
  if LPNextFreeBlock = nil then
  begin
    LPNewFirstPartiallyFreeSpan :=
      LPFirstPartiallyFreeSpan.NextPartiallyFreeSpan;
    APSmallBlockManager.FirstPartiallyFreeSpan := LPNewFirstPartiallyFreeSpan;
    LPNewFirstPartiallyFreeSpan.PreviousPartiallyFreeSpan :=
      PSmallBlockSpanHeader(APSmallBlockManager);
  end;
...
function FastMM_GetMem_GetSmallBlock(APSmallBlockManager
  : PSmallBlockManager): Pointer;
{$IFDEF X86ASM}
asm
  { --------------Attempt 1--------------
  Try to get a block from the first arena with an available block.  During the first attempt only memory that has
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        if ((APSmallBlockManager.PendingFreeList <> nil) or
          (NativeInt(APSmallBlockManager.FirstPartiallyFreeSpan) <>
          NativeInt(APSmallBlockManager))) and
          (AtomicExchange(APSmallBlockManager.SmallBlockManagerLocked, 1) = 0)
        then
        begin

          { Try to reuse a pending free block first. }
          if APSmallBlockManager.PendingFreeList <> nil then
            Exit(FastMM_GetMem_GetSmallBlock_ReusePendingFreeBlockAndUnlockArena
              (APSmallBlockManager));

          { Try to allocate a block from the first partially free span. }
          if NativeInt(APSmallBlockManager.FirstPartiallyFreeSpan) <>
            NativeInt(APSmallBlockManager) then
            Exit(FastMM_GetMem_GetSmallBlock_AllocateFreeBlockAndUnlockArena
              (APSmallBlockManager));
      ...
      if APSmallBlockManager.LastSmallBlockSequentialFeedOffset.IntegerValue > CSmallBlockSpanHeaderSize
      then
      begin
        Result := FastMM_GetMem_GetSmallBlock_TryGetBlockFromSequentialFeedSpan
          (APSmallBlockManager);
        if Result <> nil then
          Exit;
      end;
      ...
      if NativeUInt(APSmallBlockManager) <
        NativeUInt(@SmallBlockManagers[CFastMM_SmallBlockArenaCount - 1]) then
    ...
    Dec(APSmallBlockManager, CSmallBlockTypeCount *
      (CFastMM_SmallBlockArenaCount - 1));
    ...
    begin
      ...
      if AtomicCmpExchange(APSmallBlockManager.SmallBlockManagerLocked, 1, 0) = 0
      then
      begin
        ...
        Exit(FastMM_GetMem_GetSmallBlock_AllocateNewSequentialFeedSpanAndUnlockArena
          (APSmallBlockManager));
...
function FastMM_ReallocMem_ReallocSmallBlock(APointer: Pointer;
  ANewUserSize: NativeInt): Pointer;
{$IFDEF X86ASM}
asm
...
begin
  ...
  begin
    ...
    LSmallBlockTypeIndex := SmallBlockTypeLookup
      [(NativeUInt(ASize) + (CSmallBlockHeaderSize - 1))
      shr CSmallBlockGranularityBits];
  ...
  begin
    ...
    if NativeUInt(ASize) <= (CMaximumMediumBlockSize - CMediumBlockHeaderSize)
    then
    begin
...
begin
  ...
  FillChar(APointer^, FastMM_BlockMaximumUserBytes(APointer),
    CDebugFillPattern1B);
...
begin
  ...
  if (Result <> nil) and
    (ASize <= (CMaximumMediumBlockSize - CMediumBlockHeaderSize)) then
...
function FastMM_DebugGetMem_GetDebugBlock(ASize: NativeInt;
  AFillBlockWithDebugPattern: Boolean): Pointer;
var
...
begin
  ...
  Result := FastMM_GetMem(ASize + CDebugBlockHeaderSize +
    CalculateDebugBlockFooterSize(LStackTraceDepth));
  if Result = nil then
    Exit;
  ...
  begin
    FastMM_GetStackTrace(PFastMM_DebugBlockHeader(Result)
      .DebugFooter_AllocationStackTracePtr, LStackTraceDepth,
      CFastMM_StackTrace_SkipFrames_GetMem);
    FillChar(PFastMM_DebugBlockHeader(Result).DebugFooter_FreeStackTracePtr^,
      LStackTraceDepth * SizeOf(Pointer), 0);
  end;
  PFastMM_DebugBlockHeader(Result).AllocationGroup :=
    FastMM_CurrentAllocationGroup;
  PFastMM_DebugBlockHeader(Result).AllocationNumber :=
    AtomicIncrement(FastMM_LastAllocationNumber);
...
begin
  ...
  { Large blocks are already zero filled }
  if (Result <> nil) and
    (ASize <= (CMaximumMediumBlockSize - CMediumBlockHeaderSize -
    CDebugBlockHeaderSize - CalculateDebugBlockFooterSize
    (PFastMM_DebugBlockHeader(Result).StackTraceEntryCount))) then
  begin
    FillChar(Result^, ASize, 0);
  end;
end;
...
function FastMM_DebugLibrary_LegacyLogStackTrace_Wrapper(APReturnAddresses
  : PNativeUInt; AMaxDepth: Cardinal; APBufferPosition, APBufferEnd: PWideChar)
  : PWideChar;
var
  LAnsiBuffer: array [0 .. CFastMM_StackTrace_MaximumEntryCount * 256]
    of AnsiChar;
  LPEnd, LPCurPos: PAnsiChar;
begin
  Result := APBufferPosition;

  LPEnd := DebugLibrary_LogStackTrace_Legacy(APReturnAddresses, AMaxDepth,
    @LAnsiBuffer);
  LPCurPos := @LAnsiBuffer;
  while (LPCurPos < LPEnd) and (Result < APBufferEnd) do
  begin
...
begin
  ...
  begin
    LPSmallBlockSpan := GetSpanForSmallBlock(APointer);
    Result := LPSmallBlockSpan.SmallBlockManager.BlockSize -
      CSmallBlockHeaderSize;
  end
  else
  begin
...
begin
  ...
  begin
    ...
    begin
      if LBlockHeader and CIsLargeBlockFlag = CIsLargeBlockFlag then
      begin
        Result := PLargeBlockHeader(APointer)[-1].ActualBlockSize -
          CLargeBlockHeaderSize;
      end
      else
      begin
...
begin
  ...
  begin
    ...
    begin
      ...
      if LPSmallBlockManager.PendingFreeList <> nil then
      begin
        if AtomicCmpExchange(LPSmallBlockManager.SmallBlockManagerLocked, 1,
          0) = 0 then
        begin
          { Process the pending frees list. }
          LPPendingFreeBlock :=
    ...
    begin
      ...
      begin
        ...
        LPPendingFreeBlock :=
          AtomicExchange(LPMediumBlockManager.PendingFreeList, nil);
        if LPPendingFreeBlock <> nil then
          FastMM_FreeMem_FreeMediumBlockChain(LPMediumBlockManager,
            LPPendingFreeBlock, True)
        else
          LPMediumBlockManager.MediumBlockManagerLocked := 0;
      end
      else
  ...
  begin
    ...
    begin
      ...
      then
      begin
        if ProcessLargeBlockPendingFrees_ArenaAlreadyLocked
          (LPLargeBlockManager) <> 0 then
          System.Error(reInvalidPtr);
      end
      else
      begin
...
procedure FastMM_WalkBlocks_AdjustForDebugSubBlock(var ABlockInfo
  : TFastMM_WalkAllocatedBlocks_BlockInfo); inline;
begin
...
function FastMM_WalkBlocks_CheckTimeout(var ALockWaitTimeMilliseconds,
  APreviousTimestampMilliseconds: Cardinal;
...
begin
  ...
  begin
    ...
    Inc(ALockWaitTimeMilliseconds, LCurrentTimestampMilliseconds -
      APreviousTimestampMilliseconds);
  end;
...
function FastMM_WalkBlocks(ACallBack: TFastMM_WalkBlocksCallback;
  AWalkBlockTypes: TFastMM_WalkBlocksBlockTypes; AWalkUsedBlocksOnly: Boolean;
  AUserData: Pointer; ALockTimeoutMilliseconds: Cardinal): Boolean;
var
  LArenaIndex: Integer;
  ...
  LBlockOffsetFromMediumSpanStart, LMediumBlockSize, LSmallBlockOffset,
    LLastBlockOffset: Integer;
  LPSmallBlockManager: PSmallBlockManager;
begin
  ...
  if AWalkBlockTypes = [] then
    AWalkBlockTypes := [Low(TFastMM_WalkAllocatedBlocksBlockType)
      .. High(TFastMM_WalkAllocatedBlocksBlockType)];
  ...
  begin
    ...
    begin
      ...
      while (AtomicCmpExchange(LPLargeBlockManager.LargeBlockManagerLocked, 1,
        0) <> 0) and FastMM_WalkBlocks_CheckTimeout(LLockWaitTimeMilliseconds,
        LTimestampMilliseconds, ALockTimeoutMilliseconds) do
      begin
        OS_AllowOtherThreadToRun;
      end;
      ...
      begin
        LBlockInfo.BlockAddress := @PByte(LPLargeBlockHeader)
          [CLargeBlockHeaderSize];
  ...
  if AWalkBlockTypes * [btMediumBlockSpan, btMediumBlock, btSmallBlockSpan,
    btSmallBlock] <> [] then
  begin
    ...
    begin
      ...
      LLockWaitTimeMilliseconds := 0;
      while (AtomicCmpExchange(LPMediumBlockManager.MediumBlockManagerLocked, 1,
        0) <> 0) and FastMM_WalkBlocks_CheckTimeout(LLockWaitTimeMilliseconds,
        LTimestampMilliseconds, ALockTimeoutMilliseconds) do
      begin
        OS_AllowOtherThreadToRun;
      ...
      begin
        ...
        if LPMediumBlockManager.SequentialFeedMediumBlockSpan = LPMediumBlockSpan
        then
        begin
          LBlockOffsetFromMediumSpanStart :=
            LPMediumBlockManager.LastMediumBlockSequentialFeedOffset.
            IntegerValue;
          ...
          LPMediumBlock := PByte(LPMediumBlockSpan) +
            LBlockOffsetFromMediumSpanStart;
          LLockWaitTimeMilliseconds := 0;
          while (GetMediumBlockSize(LPMediumBlock) = 0) and
            FastMM_WalkBlocks_CheckTimeout(LLockWaitTimeMilliseconds,
            LTimestampMilliseconds, ALockTimeoutMilliseconds) do
          begin
            OS_AllowOtherThreadToRun;
          end;
        ...
        begin
          ...
          LBlockInfo.UsableSize := LPMediumBlockSpan.SpanSize -
            CMediumBlockSpanHeaderSize;
          ...
          begin
            ...
            LBlockInfo.MediumBlockSequentialFeedSpanUnusedBytes :=
              LBlockOffsetFromMediumSpanStart - CMediumBlockSpanHeaderSize;
          end
          else
          begin
        ...
        if AWalkBlockTypes * [btMediumBlock, btSmallBlockSpan, btSmallBlock]
          <> [] then
        begin
          ...
          begin
            ...
            begin
              ...
              if (AWalkBlockTypes * [btSmallBlockSpan, btSmallBlock] <> []) and
                PMediumBlockHeader(LPMediumBlock)[-1].IsSmallBlockSpan then
              begin
                LPSmallBlockManager := PSmallBlockSpanHeader(LPMediumBlock)
                  .SmallBlockManager;

                LLockWaitTimeMilliseconds := 0;
                while (AtomicCmpExchange
                  (LPSmallBlockManager.SmallBlockManagerLocked, 1, 0) <> 0) and
                  FastMM_WalkBlocks_CheckTimeout(LLockWaitTimeMilliseconds,
                  LTimestampMilliseconds, ALockTimeoutMilliseconds) do
                begin
                  OS_AllowOtherThreadToRun;
                ...
                begin
                  ...
                  begin
                    if LPSmallBlockManager.SequentialFeedSmallBlockSpan = LPMediumBlock
                    then
                    begin
                      LSmallBlockOffset :=
                        LPSmallBlockManager.LastSmallBlockSequentialFeedOffset.
                        IntegerValue;
              ...
              begin
                ...
                begin
                  ...
                  begin
                    ...
                    LBlockInfo.UsableSize := LPSmallBlockManager.BlockSize *
                      PSmallBlockSpanHeader(LPMediumBlock).TotalBlocksInSpan;
                    LBlockInfo.SmallBlockSpanBlockSize :=
                      LPSmallBlockManager.BlockSize;
                    LBlockInfo.IsSequentialFeedSmallBlockSpan :=
                      LSmallBlockOffset > CSmallBlockSpanHeaderSize;
                    if LBlockInfo.IsSequentialFeedSmallBlockSpan then
                      LBlockInfo.SmallBlockSequentialFeedSpanUnusedBytes :=
                        LSmallBlockOffset - CSmallBlockSpanHeaderSize
                    else
                ...
                begin
                  ...
                  begin
                    ...
                    LBlockInfo.UsableSize := LMediumBlockSize -
                      CMediumBlockHeaderSize;
                    LBlockInfo.SmallBlockSpanBlockSize := 0;
                    LBlockInfo.IsSequentialFeedSmallBlockSpan := False;
                    LBlockInfo.SmallBlockSequentialFeedSpanUnusedBytes := 0;
                    FastMM_WalkBlocks_AdjustForDebugSubBlock(LBlockInfo);
              ...
              begin
                ...
                begin
                  LLastBlockOffset := CSmallBlockSpanHeaderSize +
                    LPSmallBlockManager.BlockSize *
                    (PSmallBlockSpanHeader(LPMediumBlock)
                    .TotalBlocksInSpan - 1);
                  while LSmallBlockOffset <= LLastBlockOffset do
                  begin
                    LBlockInfo.BlockAddress := PByte(LPMediumBlock) +
                      LSmallBlockOffset;

                    LBlockInfo.BlockIsFree :=
                      BlockIsFree(LBlockInfo.BlockAddress);
                    if (not AWalkUsedBlocksOnly) or (not LBlockInfo.BlockIsFree)
                    then
                    begin
                      LBlockInfo.BlockSize := LPSmallBlockManager.BlockSize;
                      LBlockInfo.UsableSize := LPSmallBlockManager.BlockSize -
                        CSmallBlockHeaderSize;
                      LBlockInfo.ArenaIndex :=
                        (NativeInt(LPSmallBlockManager) -
                        NativeInt(@SmallBlockManagers))
                        div SizeOf(TSmallBlockArena);
...
procedure FastMM_ScanDebugBlocksForCorruption_CallBack(const ABlockInfo
  : TFastMM_WalkAllocatedBlocks_BlockInfo);
begin
  ...
  if (ABlockInfo.DebugInformation.CalculateHeaderCheckSum <>
    ABlockInfo.DebugInformation.HeaderCheckSum) or
    (ABlockInfo.DebugInformation.CalculateFooterCheckSum <>
    ABlockInfo.DebugInformation.DebugFooterPtr^) then
  begin
    ...
    begin
      ...
      if not CheckDebugBlockHeaderAndFooterCheckSumsValid
        (ABlockInfo.DebugInformation) then
        System.Error(reInvalidPtr);
    end
    else
    begin
  ...
  if ABlockInfo.BlockIsFree and
    (not CheckDebugBlockFillPatternIntact(ABlockInfo.DebugInformation)) then
    System.Error(reInvalidPtr);
end;
...
function FastMM_ScanDebugBlocksForCorruption(ALockTimeoutMilliseconds
  : Cardinal): Boolean;
begin
  FastMM_WalkBlocks(FastMM_ScanDebugBlocksForCorruption_CallBack,
    [btLargeBlock, btMediumBlock, btSmallBlock], False, nil,
    ALockTimeoutMilliseconds);

  Result := True;
end;

procedure FastMM_GetHeapStatus_CallBack(const ABlockInfo
  : TFastMM_WalkAllocatedBlocks_BlockInfo);
var
    ...
    btMediumBlockSpan:
      begin
        ...
        if ABlockInfo.IsSequentialFeedMediumBlockSpan then
          Inc(LPHeapStatus.Unused,
            ABlockInfo.MediumBlockSequentialFeedSpanUnusedBytes);
      end;
    ...
    btSmallBlockSpan:
      begin
        if ABlockInfo.IsSequentialFeedSmallBlockSpan then
          Inc(LPHeapStatus.Unused,
            ABlockInfo.SmallBlockSequentialFeedSpanUnusedBytes);
      end;
...
begin
  ...
  FastMM_WalkBlocks(FastMM_GetHeapStatus_CallBack,
    [btLargeBlock, btMediumBlockSpan, btMediumBlock, btSmallBlockSpan,
    btSmallBlock], False, @Result, ALockTimeoutMilliseconds);

  Result.TotalFree := Result.FreeSmall + Result.FreeBig + Result.Unused;
  Result.TotalAddrSpace := Result.TotalCommitted;
  Result.Overhead := Result.TotalAddrSpace - Result.TotalAllocated -
    Result.TotalFree;
end;

function FastMM_GetUsageSummary(ALockTimeoutMilliseconds: Cardinal)
  : TFastMM_UsageSummary;
var
...
begin
  ...
  Result.OverheadBytes := LHeapStatus.TotalAddrSpace -
    LHeapStatus.TotalAllocated;

  if LHeapStatus.TotalAddrSpace > 0 then
    Result.EfficiencyPercentage := Result.AllocatedBytes /
      LHeapStatus.TotalAddrSpace * 100
  else
...
begin
  ...
  begin
    ...
    if NativeUInt(LPMediumBlockManager.FirstMediumBlockSpanHeader) <>
      NativeUInt(LPMediumBlockManager) then
      Exit(True);
  end;
  ...
  begin
    ...
    if NativeUInt(LPLargeBlockManager.FirstLargeBlockHeader) <>
      NativeUInt(LPLargeBlockManager) then
      Exit(True);
  end;
...
begin
  ...
  Result := (@LCurrentMemoryManager.GetMem <> @InstalledMemoryManager.GetMem) or
    (@LCurrentMemoryManager.FreeMem <> @InstalledMemoryManager.FreeMem) or
    (@LCurrentMemoryManager.ReallocMem <> @InstalledMemoryManager.ReallocMem) or
    (@LCurrentMemoryManager.AllocMem <> @InstalledMemoryManager.AllocMem) or
    (@LCurrentMemoryManager.RegisterExpectedMemoryLeak <>
    @InstalledMemoryManager.RegisterExpectedMemoryLeak) or
    (@LCurrentMemoryManager.UnregisterExpectedMemoryLeak <>
    @InstalledMemoryManager.UnregisterExpectedMemoryLeak);
end;
...
procedure FastMM_LogStateToFile_Callback(const ABlockInfo
  : TFastMM_WalkAllocatedBlocks_BlockInfo);
var
...
begin
  ...
  if (LBlockContentType > 65535) and
    (LBlockContentType and (SizeOf(Pointer) - 1) = 0) then
  begin
    ...
    repeat
      ...
      if (LPClassNode = nil) or
        (LPClassNode.BlockContentType = LBlockContentType) then
        Break;
  ...
  begin
    ...
    LBlockContentType := DetectBlockContentType(ABlockInfo.BlockAddress,
      ABlockInfo.UsableSize);
    ...
    repeat
      ...
      begin
        ...
        LPParentNode.LeftAndRightNodePointers[LChildNodeDirection] :=
          LPClassNode;
...
procedure FastMM_LogStateToFile_QuickSortLogNodes(APLeftItem: PMemoryLogNodes;
  ARightIndex: Integer);
var
  M, i, J: Integer;
...
begin
  ...
  begin
    ...
    if APLeftItem[M].TotalMemoryUsage < APLeftItem[ARightIndex].TotalMemoryUsage
    then
    begin
...
procedure FastMM_LogStateToFile_InsertionSortLogNodes
  (APLeftItem: PMemoryLogNodes; ARightIndex: Integer);
var
  i, J: Integer;
...
begin
  ...
  begin
    ...
    J := i;
    while (J > 0) and (APLeftItem[J - 1].TotalMemoryUsage <
      LCurNode.TotalMemoryUsage) do
    begin
...
function FastMM_LogStateToFile(const AFilename: string;
  const AAdditionalDetails: string; ALockTimeoutMilliseconds: Cardinal)
  : Boolean;
const
...
var
  ...
  LBufferSize: Integer;
  LPLogInfo: PMemoryLogInfo;
  LPTokenBufferStart, LPStateLogBufferStart, LPBufferEnd, LPTokenPos,
    LPStateLogPos: PWideChar;
  LTokenValues: TEventLogTokenValues;
  LInd: Integer;
...
begin
  ...
  LBufferSize := SizeOf(TMemoryLogInfo) +
    (CTokenBufferMaxWideChars + CStateLogMaxChars) * SizeOf(Char);
  ...
  begin
    try
      ...
      FastMM_WalkBlocks(FastMM_LogStateToFile_Callback,
        [btLargeBlock, btMediumBlock, btSmallBlock], True, LPLogInfo,
        ALockTimeoutMilliseconds);
      ...
      if LPLogInfo.NodeCount >= CQuickSortMinimumItemsInPartition then
        FastMM_LogStateToFile_QuickSortLogNodes(@LPLogInfo.Nodes[0],
          LPLogInfo.NodeCount - 1);
      FastMM_LogStateToFile_InsertionSortLogNodes(@LPLogInfo.Nodes[0],
        LPLogInfo.NodeCount - 1);
      ...
      LTokenValues := Default (TEventLogTokenValues);
      LPTokenPos := AddTokenValues_GeneralTokens(LTokenValues,
        LPTokenBufferStart, LPStateLogBufferStart);
      LPTokenPos := AddTokenValue_NativeUInt(LTokenValues,
        CStateLogTokenAllocatedKB,
        LMemoryManagerUsageSummary.AllocatedBytes div 1024, LPTokenPos,
        LPStateLogBufferStart);
      LPTokenPos := AddTokenValue_NativeUInt(LTokenValues,
        CStateLogTokenOverheadKB,
        LMemoryManagerUsageSummary.OverheadBytes div 1024, LPTokenPos,
        LPStateLogBufferStart);
      AddTokenValue_NativeInt(LTokenValues, CStateLogTokenEfficiencyPercentage,
        Round(LMemoryManagerUsageSummary.EfficiencyPercentage), LPTokenPos,
        LPStateLogBufferStart);
      LPStateLogPos := SubstituteTokenValues(FastMM_LogStateToFileTemplate,
        LTokenValues, LPStateLogBufferStart, LPBufferEnd);
      ...
      begin
        ...
        LPTokenPos := AddTokenValue_NativeUInt(LTokenValues,
          CStateLogTokenClassTotalBytesUsed, LPNode.TotalMemoryUsage,
          LPTokenBufferStart, LPStateLogBufferStart);
        LPTokenPos := AddTokenValue_NativeUInt(LTokenValues,
          CStateLogTokenClassInstanceCount, LPNode.InstanceCount, LPTokenPos,
          LPStateLogBufferStart);
        LPTokenPos := AddTokenValue_NativeUInt(LTokenValues,
          CStateLogTokenClassAverageBytesPerInstance,
          Round(LPNode.TotalMemoryUsage / LPNode.InstanceCount), LPTokenPos,
          LPStateLogBufferStart);
        AddTokenValue_BlockContentType(LTokenValues, CEventLogTokenObjectClass,
          LPNode.BlockContentType, LPTokenPos, LPStateLogBufferStart);
        LPStateLogPos := SubstituteTokenValues
          (FastMM_LogStateToFileTemplate_UsageDetail, LTokenValues,
          LPStateLogPos, LPBufferEnd);
      end;

      { Append the additional information }
      if AAdditionalDetails <> '' then
      begin
        LPStateLogPos := AppendTextToBuffer(CRLF, 2, LPStateLogPos,
          LPBufferEnd);
        LPStateLogPos := AppendTextToBuffer(PWideChar(AAdditionalDetails),
          length(AAdditionalDetails), LPStateLogPos, LPBufferEnd);
      end;
      ...
      begin
        Result := AppendTextFile(LFileHandle, LPStateLogBufferStart,
          CharCount(LPStateLogPos, LPStateLogBufferStart));
...
begin
  ...
  begin
    ...
    LPMapAddress := MapViewOfFile(LLocalMappingObjectHandle,
      FILE_MAP_READ, 0, 0, 0);
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        LTokenValues := Default (TEventLogTokenValues);
        AddTokenValues_GeneralTokens(LTokenValues, @LTokenValueBuffer,
          @LTokenValueBuffer[High(LTokenValueBuffer)]);
        LogEvent(mmetCannotSwitchToSharedMemoryManagerWithLivePointers,
          LTokenValues);
...
var
  LPMapAddress: Pointer;
begin
  if (CurrentInstallationState = mmisInstalled) and
    (not FastMM_InstalledMemoryManagerChangedExternally) and
    (SharingFileMappingObjectHandle = 0) then
  begin
    { Is any other module already sharing its MM? }
    if FastMM_FindSharedMemoryManager = nil then
    begin
      { Create the memory mapped file }
      SharingFileMappingObjectHandle := CreateFileMappingA(INVALID_HANDLE_VALUE,
        nil, PAGE_READWRITE, 0, SizeOf(Pointer), SharingFileMappingObjectName);
      { Map a view of the memory }
      LPMapAddress := MapViewOfFile(SharingFileMappingObjectHandle,
...
begin
  ...
  begin
    ...
    if (LPInsertAfter.LeakAddress = APNewEntry.LeakAddress) and
      ((LPInsertAfter.LeakedClass = APNewEntry.LeakedClass)) and
      ((LPInsertAfter.LeakSize = APNewEntry.LeakSize) or
      ((not AExactSizeMatch) and (LPInsertAfter.LeakSize < APNewEntry.LeakSize)
      and ((LPInsertAfter.NextLeak = nil) or (LPInsertAfter.NextLeak.LeakSize >
  ...
  begin
    ...
    begin
      if ExpectedMemoryLeaks.EntriesUsed <
        length(ExpectedMemoryLeaks.ExpectedLeaks) then
      begin
        LPNewEntry := @ExpectedMemoryLeaks.ExpectedLeaks
          [ExpectedMemoryLeaks.EntriesUsed];
...
begin
  ...
  if ExpectedMemoryLeaks = nil then
    ExpectedMemoryLeaks := OS_AllocateVirtualMemory
      (CExpectedMemoryLeaksListSize, False);
...
begin
  ...
  Result := LockExpectedMemoryLeaksList and
    UpdateExpectedLeakList(@ExpectedMemoryLeaks.FirstEntryByAddress,
    @LNewEntry);
...
begin
  ...
  { Add it to the correct list }
  Result := LockExpectedMemoryLeaksList and
    UpdateExpectedLeakList(@ExpectedMemoryLeaks.FirstEntryBySizeOnly,
    @LNewEntry);
  ExpectedMemoryLeaksListLocked := 0;
end;
...
function FastMM_PerformMemoryLeakCheck_DetectLeakType(AAddress: Pointer;
  ASpaceInsideBlock: NativeInt): TMemoryLeakType;
var
...
begin
  ...
  begin
    ...
    LLeak.LeakSize := 0;
    LLeak.LeakCount := -1;
    if UpdateExpectedLeakList(@ExpectedMemoryLeaks.FirstEntryByAddress, @LLeak,
      False) then
    begin
    ...
    if UpdateExpectedLeakList(@ExpectedMemoryLeaks.FirstEntryByClass, @LLeak,
      False) then
    begin
    ...
    LLeak.LeakedClass := nil;
    if UpdateExpectedLeakList(@ExpectedMemoryLeaks.FirstEntryBySizeOnly, @LLeak,
      False) then
      Result := mltExpectedLeakRegisteredBySize;
  end;
end;

procedure FastMM_PerformMemoryLeakCheck_AddBlockToLeakSummary
  (APLeakSummary: PMemoryLeakSummary; ABlockUsableSize: NativeInt;
  ABlockContentType: NativeUInt);
var
...
begin
  ...
  begin
{$IF CompilerVersion < 31}
    LChildDirection := False;
    // Workaround for spurious warning with older compilers
{$ENDIF}
    while True do
    begin
...
procedure FastMM_PerformMemoryLeakCheck_CallBack(const ABlockInfo
  : TFastMM_WalkAllocatedBlocks_BlockInfo);
var
...
begin
  ...
  if FastMM_PerformMemoryLeakCheck_DetectLeakType(ABlockInfo.BlockAddress,
    ABlockInfo.UsableSize) <> mltUnexpectedLeak then
    Exit;
  ...
  begin
    ...
    LPBufferEnd := @LTokenValueBuffer[High(LTokenValueBuffer)];
    LPBufferPos := AddTokenValues_GeneralTokens(LTokenValues,
      @LTokenValueBuffer, LPBufferEnd);
    AddTokenValues_BlockTokens(LTokenValues, ABlockInfo.BlockAddress,
      LPBufferPos, LPBufferEnd);
  ...
  LBlockContentType := DetectBlockContentType(ABlockInfo.BlockAddress,
    ABlockInfo.UsableSize);
  FastMM_PerformMemoryLeakCheck_AddBlockToLeakSummary(LPLeakSummary,
    ABlockInfo.UsableSize, LBlockContentType);
end;

procedure FastMM_PerformMemoryLeakCheck_SortNodes(var ALeakSummary
  : TMemoryLeakSummary);
var
...
begin
  ...
  begin
    ...
    begin
      if ALeakSummary.MemoryLeakEntries[LInsertionIndex - 1].BlockUsableSize <
        LCurEntry.BlockUsableSize then
        Break;

      if (ALeakSummary.MemoryLeakEntries[LInsertionIndex - 1]
        .BlockUsableSize = LCurEntry.BlockUsableSize) and
        (ALeakSummary.MemoryLeakEntries[LInsertionIndex - 1].BlockContentType >
        LCurEntry.BlockContentType) then
      begin
        Break;
      end;

      ALeakSummary.MemoryLeakEntries[LInsertionIndex] :=
        ALeakSummary.MemoryLeakEntries[LInsertionIndex - 1];
      Dec(LInsertionIndex);
    end;
...
procedure FastMM_PerformMemoryLeakCheck_LogLeakSummary(var ALeakSummary
  : TMemoryLeakSummary);
const
...
var
  ...
  LLeakIndex: Integer;
  LLeakEntriesText, LTokenValueBuffer: array [0 .. CLeakTextMaxSize]
    of WideChar;
...
begin
  ...
  begin
    ...
    if ALeakSummary.MemoryLeakEntries[LLeakIndex].BlockUsableSize <> LCurrentLeakSize
    then
    begin
      LCurrentLeakSize := ALeakSummary.MemoryLeakEntries[LLeakIndex]
        .BlockUsableSize;

      LPBufferPos := AppendTextToBuffer(CLifeFeed, length(CLifeFeed),
        LPBufferPos, LPBufferEnd);
      LPBufferPos := NativeIntToTextBuffer(LCurrentLeakSize, LPBufferPos,
        LPBufferEnd);
      LPBufferPos := AppendTextToBuffer(CLeakSizeSuffix,
        length(CLeakSizeSuffix), LPBufferPos, LPBufferEnd);
    end
    else
    begin
      LPBufferPos := AppendTextToBuffer(CLeakSeparator, length(CLeakSeparator),
        LPBufferPos, LPBufferEnd);
    end;

    LPBufferPos := NativeIntToTextBuffer(ALeakSummary.MemoryLeakEntries
      [LLeakIndex].NumLeaks, LPBufferPos, LPBufferEnd);
    LPBufferPos := AppendTextToBuffer(CLeakMultiple, length(CLeakMultiple),
      LPBufferPos, LPBufferEnd);
    LPBufferPos := BlockContentTypeToTextBuffer
      (ALeakSummary.MemoryLeakEntries[LLeakIndex].BlockContentType, LPBufferPos,
      LPBufferEnd);
  end;

  { Build the token dictionary for the leak summary. }
  LTokenValues := Default (TEventLogTokenValues);
  LPTokenBufferPos := AddTokenValues_GeneralTokens(LTokenValues,
    @LTokenValueBuffer, @LTokenValueBuffer[High(LTokenValueBuffer)]);
  AddTokenValue(LTokenValues, CEventLogTokenLeakSummaryEntries,
    @LLeakEntriesText, CharCount(LPBufferPos, @LLeakEntriesText),
    LPTokenBufferPos, @LTokenValueBuffer[High(LTokenValueBuffer)]);
...
begin
  ...
  FastMM_WalkBlocks(FastMM_PerformMemoryLeakCheck_CallBack,
    [btLargeBlock, btMediumBlock, btSmallBlock], True, @LLeakSummary,
    CFastMM_PerformMemoryLeakCheck_LockTimeout);
...
function SmallBlockManagerIndexFromSizeIndex(ASizeIndex: Integer)
  : Integer; inline;
begin
...
var
  LBlockSizeIndex, LSmallBlockSize, LManagerIndex, LStartIndex, LNextStartIndex,
    LAndValue: Integer;
begin
  ...
  if AlignmentRequestCounters[maa64Bytes] > 0 then
    ...
    LAndValue := 31
  else if (SizeOf(Pointer) = 8) or (AlignmentRequestCounters[maa16Bytes] > 0)
  then
    LAndValue := 15
  else
    LAndValue := 0;
...
procedure FastMM_SetMemoryUsageLimit(AMaximumAllowedMemoryUsage: NativeUInt);
const
  CMemoryUsageLimitGraceAmount = 32 * 1024 * 1024;
  // Hopefully enough to allow shutdown code to run
begin
...
function FastMM_InitializeMemoryManager_GetOptimalMoveProc(ASmallBlockSize
  : Integer): TMoveProc;
{$IFDEF X86ASM}
var
...
begin
{$IFDEF X86ASM}
  LSSE2Available := System.TestSSE and 2 <> 0;
  // Bit 1 = 1 means the CPU supports SSE2
{$ENDIF}
...
var
  LBlockSizeIndex, LSmallBlockSize, LArenaInd, LMinimumSmallBlockSpanSize,
    LBinInd, LOptimalSmallBlockSpanSize, LBlocksPerSpan, LManagerIndex: Integer;
  LPSmallBlockManager: PSmallBlockManager;
  LPMediumBlockManager: PMediumBlockManager;
  LPLargeBlockManager: PLargeBlockManager;
  LPBin: PPointer;
...
{$IF CLargeBlockManagerSize and 63 <> 0} {$MESSAGE error 'Large block manager size must be a multiple of 64 bytes'} {$ENDIF}
  ...
  begin
    ...
    LMinimumSmallBlockSpanSize := RoundUserSizeUpToNextMediumBlockBin
      (CMinimumSmallBlocksPerSpan * LSmallBlockSize +
    ...
    LOptimalSmallBlockSpanSize := RoundUserSizeUpToNextMediumBlockBin
      (LBlocksPerSpan * LSmallBlockSize + (CSmallBlockSpanHeaderSize -
      CSmallBlockHeaderSize));
    ...
    begin
      ...
      LPSmallBlockManager.FirstPartiallyFreeSpan :=
        PSmallBlockSpanHeader(LPSmallBlockManager);
      LPSmallBlockManager.LastPartiallyFreeSpan :=
        PSmallBlockSpanHeader(LPSmallBlockManager);

      LPSmallBlockManager.LastSmallBlockSequentialFeedOffset.
        IntegerAndABACounter := 0;
      ...
      LPSmallBlockManager.UpsizeMoveProcedure :=
        FastMM_InitializeMemoryManager_GetOptimalMoveProc
        (LPSmallBlockManager.BlockSize);
  ...
  begin
    ...
    LPMediumBlockManager.FirstMediumBlockSpanHeader :=
      PMediumBlockSpanHeader(LPMediumBlockManager);
    LPMediumBlockManager.LastMediumBlockSpanHeader :=
      PMediumBlockSpanHeader(LPMediumBlockManager);
  ...
  begin
    ...
    LPLargeBlockManager.FirstLargeBlockHeader :=
      PLargeBlockHeader(LPLargeBlockManager);
    LPLargeBlockManager.LastLargeBlockHeader :=
      PLargeBlockHeader(LPLargeBlockManager)
  end;
...
begin
  ...
  begin
    ...
    FillChar(LPMediumBlockManager.MediumBlockBinBitmaps,
      SizeOf(LPMediumBlockManager.MediumBlockBinBitmaps), 0);
    for LBinIndex := 0 to CMediumBlockBinCount - 1 do
      LPMediumBlockManager.FirstFreeBlockInBin[LBinIndex] :=
        @LPMediumBlockManager.FirstFreeBlockInBin[LBinIndex];
...
begin
  ...
  begin
    ...
    LNewMemoryManager.RegisterExpectedMemoryLeak :=
      FastMM_RegisterExpectedMemoryLeak;
    LNewMemoryManager.UnregisterExpectedMemoryLeak :=
      FastMM_UnregisterExpectedMemoryLeak;
  end
  else
  begin
...
begin
  ...
  begin
    ...
    AddTokenValues_GeneralTokens(LTokenValues, @LTokenValueBuffer,
      @LTokenValueBuffer[High(LTokenValueBuffer)]);
    LogEvent(mmetCannotInstallAfterDefaultMemoryManagerHasBeenUsed,
      LTokenValues);
...
begin
  if CurrentInstallationState in [mmisInstalled, mmisUsingSharedMemoryManager]
  then
  begin
...
{$IFNDEF FastMM_DebugLibraryStaticDependency}
  ...
  if DebugSupportLibraryHandle <> 0 then
  begin
    DebugLibrary_GetRawStackTrace := GetProcAddress(DebugSupportLibraryHandle,
      PAnsiChar('GetRawStackTrace'));
    DebugLibrary_GetFrameBasedStackTrace :=
      GetProcAddress(DebugSupportLibraryHandle,
      PAnsiChar('GetFrameBasedStackTrace'));
    DebugLibrary_LogStackTrace_Legacy :=
      GetProcAddress(DebugSupportLibraryHandle, PAnsiChar('LogStackTrace'));

    { Try to use the stack trace routines from the debug support library, if available. }
    if (@FastMM_GetStackTrace = @FastMM_NoOpGetStackTrace) and
      Assigned(DebugLibrary_GetRawStackTrace) then
    begin
    ...
    begin
      FastMM_ConvertStackTraceToText :=
        FastMM_DebugLibrary_LegacyLogStackTrace_Wrapper;
    end;
...
{$IFNDEF FastMM_DebugLibraryStaticDependency}
  if DebugSupportLibraryHandle = 0 then
    Exit(False);
{$ENDIF}
  if (@FastMM_GetStackTrace = @DebugLibrary_GetRawStackTrace) or
    (@FastMM_GetStackTrace = @DebugLibrary_GetFrameBasedStackTrace) then
  begin
    FastMM_GetStackTrace := @FastMM_NoOpGetStackTrace;
  end;
...
begin
  ...
  if (@FastMM_GetStackTrace = @FastMM_NoOpGetStackTrace) or
    (@FastMM_ConvertStackTraceToText = @FastMM_NoOpConvertStackTraceToText) then
  begin
...
var
  ...
  LPModuleFilenamePos, LPModuleFilenameStart, LPModuleFilenameEnd, LPBufferPos,
    LPBufferEnd: PWideChar;
begin
  CloseEventLogFile;

  { Get the module filename into a buffer. }
  LPModuleFilenameEnd := OS_GetApplicationFilename(@LModuleFilename,
    @LModuleFilename[High(LModuleFilename)], False);
  ...
  LPBufferPos := OS_GetEnvironmentVariableValue(CLogFilePathEnvironmentVariable,
    @EventLogFilename, LPBufferEnd);
  ...
  LPBufferPos := AppendTextToBuffer(LPModuleFilenameStart, LPBufferPos,
    LPBufferEnd);
  LPBufferPos := AppendTextToBuffer(CLogFileExtension, LPBufferPos,
    LPBufferEnd);
  LPBufferPos^ := #0;
end;
...
begin
  ...
  begin
    LPBufferEnd := @EventLogFilename[High(EventLogFilename)];
    LPBufferPos := AppendTextToBuffer(APEventLogFilename, @EventLogFilename,
      LPBufferEnd);
    LPBufferPos^ := #0;
  end
  else
